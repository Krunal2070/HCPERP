...
# Hidden anchor stubs the page JS shows/polls (FG link + GRN/MTV tabs + badges).
# Kept inside the sidebar <nav> via sidebar_extra_html so existing JS keeps working.
_PROC_SIDEBAR_EXTRA = """
<div style="display:none" id="procHiddenHooks">
  <a class="sidebar-item" id="sb-fg" href="/fg"></a>
  <a class="sidebar-item" id="sb-grn" onclick="switchTab('grn');setSidebarActive('grn')"><span class="sidebar-badge" id="sbBadge-grn">–</span></a>
  <a class="sidebar-item" id="sb-mtv" onclick="switchTab('mtv');setSidebarActive('mtv');if(typeof loadMtvList==='function'&&!window._mtvLoaded){window._mtvLoaded=true;loadMtvList();}"><span class="sidebar-badge" id="sbBadge-mtv">–</span></a>
</div>
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# PO APPROVAL  —  permission helpers
# ─────────────────────────────────────────────────────────────────────────────

def _current_username() -> str:
    """Returns current logged-in user's username (lowercased), '' if not logged in."""
    return (session.get("UID") or "").strip().lower()


# ─────────────────────────────────────────────────────────────────────────────
# CREDENTIAL ENCRYPTION  —  Fernet (symmetric AES) for Gmail app passwords
# ─────────────────────────────────────────────────────────────────────────────
#
# Strategy: derive a Fernet key from either:
#   1. Environment variable PROCUREMENT_SECRET_KEY (preferred — set in production)
#   2. Flask app's SECRET_KEY (fallback — keeps things working if env var missing)
#   3. A hardcoded sentinel (last-resort fallback so the app still boots; logged)
#
# Stored passwords carry a `password_encrypted` flag so the system knows
# whether to decrypt or treat the value as legacy plaintext. New saves always
# encrypt; legacy plaintext is auto-encrypted on first access (lazy migration).

_FERNET_INSTANCE = None
_FERNET_INIT_TRIED = False

def _get_fernet():
    """Return a Fernet instance, lazily initialised. Returns None if the
    cryptography library is unavailable, in which case storage falls back
    to plaintext (with a printed warning). The returned object is cached."""
    global _FERNET_INSTANCE, _FERNET_INIT_TRIED
    if _FERNET_INIT_TRIED:
        return _FERNET_INSTANCE
    _FERNET_INIT_TRIED = True
    try:
        import os as _os, base64, hashlib
        from cryptography.fernet import Fernet
    except Exception:
        print("[procurement] WARNING: cryptography library not installed; "
              "Gmail app passwords will be stored as plaintext. "
              "Install with: pip install cryptography")
        _FERNET_INSTANCE = None
        return None
    # Source the key material
    src = _os.environ.get("PROCUREMENT_SECRET_KEY", "") or ""
    if not src:
        # Fallback to Flask's secret key if available
        try:
            from flask import current_app
            src = (current_app.config.get("SECRET_KEY") or "") if current_app else ""
        except Exception:
            src = ""
    if not src:
        # Last-resort: a known constant — clearly weaker, but keeps app functional
        print("[procurement] WARNING: no PROCUREMENT_SECRET_KEY or Flask SECRET_KEY set; "
              "using a fallback key. Set PROCUREMENT_SECRET_KEY env var in production.")
        src = "hcp-wellness-procurement-default-fallback-key-do-not-use-in-prod"
    # Derive a 32-byte key from arbitrary-length source via SHA-256, then base64 for Fernet
    key_bytes = hashlib.sha256(src.encode("utf-8")).digest()
    fernet_key = base64.urlsafe_b64encode(key_bytes)
    try:
        _FERNET_INSTANCE = Fernet(fernet_key)
    except Exception as e:
        print(f"[procurement] WARNING: Fernet init failed: {e}; falling back to plaintext")
        _FERNET_INSTANCE = None
    return _FERNET_INSTANCE


def _encrypt_secret(plaintext: str) -> tuple[str, bool]:
    """Encrypt a string. Returns (stored_value, is_encrypted).
    If Fernet is unavailable, returns (plaintext, False) so storage still works."""
    if not plaintext:
        return ("", False)
    f = _get_fernet()
    if not f:
    ...
    try:
        ...
        print(f"[procurement] WARNING: encrypt failed: {e}; storing as plaintext")
        return (plaintext, False)


def _decrypt_secret(stored: str, is_encrypted: bool) -> str:
    """Decrypt a stored secret. If the flag says not encrypted, returns as-is
    (legacy plaintext). Returns '' on decrypt failure (caller should handle)."""
    if not stored:
        return ""
    if not is_encrypted:
    ...
    f = _get_fernet()
    if not f:
        # Encrypted but no key available — can't decrypt
        print("[procurement] WARNING: stored secret is encrypted but Fernet unavailable")
        return ""
    try:
        return f.decrypt(stored.encode("ascii")).decode("utf-8")
    except Exception as e:
        print(f"[procurement] WARNING: decrypt failed: {e}")
        return ""


def _is_admin_user() -> bool:
    """The special user whose username is literally 'admin' has full powers."""
    return _current_username() == "admin"


def _can_approve_po(conn=None) -> bool:
    """
    True if the current user can approve POs:
      - is the special 'admin' user, OR
      - is listed in procurement_po_approvers
    Pass an open conn to avoid reconnecting; otherwise a fresh one is used.
    """
    if not session.get("logged_in"):
        return False
    if _is_admin_user():
    ...
    try:
        if own_conn:
            ...
            if not conn:
                return False
        row = conn.execute(
            "SELECT 1 AS ok FROM procurement_po_approvers WHERE LOWER(TRIM(username))=%s LIMIT 1",
            (uname,)
        ).fetchone()
        return bool(row)
    except Exception:
...
# ─────────────────────────────────────────────────────────────────────────────
# GSTIN → state autoderive (silent backfill on every page load)
# ─────────────────────────────────────────────────────────────────────────────

# Indian GST state codes (first 2 digits of GSTIN).
# Reference: https://www.gst.gov.in (state code list)
_GST_STATE_CODES = {
    "01": "Jammu & Kashmir",
    "02": "Himachal Pradesh",
    "03": "Punjab",
    ...
    "27": "Maharashtra",
    "28": "Andhra Pradesh",  # old code; new is 37
    "29": "Karnataka",
    "30": "Goa",
    "31": "Lakshadweep",
    "32": "Kerala",
...
def _state_from_gstin(gstin: str) -> str:
    """Extract state name from a GSTIN. Returns '' if can't determine."""
    g = (gstin or "").strip().upper()
    if len(g) < 2:
        return ""
    code = g[:2]
    return _GST_STATE_CODES.get(code, "")


def _autoderive_supplier_states():
    """
    One-shot backfill: for suppliers where state is empty but GSTIN is set,
    derive state from the GSTIN's first 2 digits and UPDATE in place.
    Called from page-render endpoint(s) so it runs on every page refresh until
    everyone's filled in. Cheap (one SELECT + at most N UPDATEs per refresh).
    Failures are silent — this is a convenience, not a critical path.
    """
    try:
        conn = sampling_portal.get_db_connection()
        if not conn:
        ...
        try:
            rows = conn.execute("""
                SELECT id, gst_number FROM procurement_suppliers
                WHERE (state IS NULL OR TRIM(state) = '')
                  AND gst_number IS NOT NULL AND TRIM(gst_number) != ''
            """).fetchall()
            updated = 0
            for r in rows:
                st = _state_from_gstin(r["gst_number"])
                if st:
                    conn.execute(
                        "UPDATE procurement_suppliers SET state=%s WHERE id=%s",
                        (st, r["id"])
                    )
                    updated += 1
            if updated:
        ...
        pass  # silent — convenience feature
...
def _init_procurement_table():
    ...
    try:
        ...
        for _col, _def in [
            ...
            ("state",           "VARCHAR(100) DEFAULT NULL"),  # for inter-state GST detection (CGST/SGST vs IGST)
        ]:
            try:
        ...
        for _col, _def in [
            ...
            ("approved_by",       "VARCHAR(100) DEFAULT NULL"),       # username (matches User_Tbl.username)
        ...
        # PO approvers table (admin grants approval rights to other users)
        try:
            conn.execute("""
                CREATE TABLE IF NOT EXISTS procurement_po_approvers (
        ...
        # Per-user email settings (procurement-specific; kept out of shared User_Tbl).
        # Stores Gmail SMTP creds + default cc/subject/body/signature for the PO email feature.
        try:
            conn.execute("""
                CREATE TABLE IF NOT EXISTS procurement_user_email_settings (
                    username              VARCHAR(100) NOT NULL PRIMARY KEY,
                    gmail_address         VARCHAR(200) DEFAULT NULL,
                    gmail_app_password    VARCHAR(256) DEFAULT NULL,
                    email_default_cc      TEXT         DEFAULT NULL,
                    email_default_subject TEXT         DEFAULT NULL,
                    email_default_body    TEXT         DEFAULT NULL,
        ...
        # Migration: add WhatsApp default template column to email settings
        # (procurement-specific personal messaging defaults — same table)
        for _col, _def in [
            ("whatsapp_default_msg", "TEXT DEFAULT NULL"),
            ("password_encrypted",   "TINYINT(1) DEFAULT 0"),  # 1 = stored gmail_app_password is Fernet-encrypted
        ]:
            try:
                conn.execute(f"ALTER TABLE procurement_user_email_settings ADD COLUMN {_col} {_def}")
                conn.commit()
            except Exception:
                pass

        # ── ONE-TIME: grandfather older POs as approved ──
        # POs created before the approval-system rollout would otherwise be stuck in
        # 'pending' and become un-printable. We mark them approved exactly once.
        # Idempotent: only touches rows that haven't been set yet AND were created
        # before today. After the rollout day, new POs go through approval normally.
        try:
            conn.execute("""
        ...
        # Email send audit log — every PO email send (success or failure) is recorded
        try:
            conn.execute("""
                CREATE TABLE IF NOT EXISTS procurement_po_email_log (
                    id          INT AUTO_INCREMENT PRIMARY KEY,
                    po_id       INT          DEFAULT NULL,
                    po_num      VARCHAR(100) DEFAULT NULL,
                    sent_by     VARCHAR(100) DEFAULT NULL,
                    sent_at     DATETIME     DEFAULT CURRENT_TIMESTAMP,
                    to_addr     TEXT         DEFAULT NULL,
                    cc_addr     TEXT         DEFAULT NULL,
        ...
        for _col, _def in [
            ...
            ("cgst_amount", "DECIMAL(18,4) DEFAULT NULL"),
            ("sgst_amount", "DECIMAL(18,4) DEFAULT NULL"),
            ("igst_amount", "DECIMAL(18,4) DEFAULT NULL"),  # for inter-state POs (mutually exclusive with cgst/sgst)
            ("discount",    "DECIMAL(7,4) DEFAULT NULL"),   # per-line discount percentage (e.g. 5.00 = 5%)
            ("packages",    "INT DEFAULT NULL"),
            ("qty_per_pkg", "DECIMAL(15,3) DEFAULT NULL"),
            ("uom",         "VARCHAR(50) DEFAULT NULL"),
        ]:
            try:
                ...
                pass
        # Migration: add overall order-level discount to PO header if missing
        try:
            conn.execute("ALTER TABLE procurement_purchase_orders ADD COLUMN order_discount DECIMAL(15,4) DEFAULT NULL")
            conn.commit()
        except Exception:
            pass
...
def register_procurement(app):
    ...
    def procurement_main():
        # Silent backfill: derive supplier states from their GSTINs (idempotent)
        _autoderive_supplier_states()
        _is_admin = (session.get('UID', '') or '').lower() == 'admin'
        return render_template(
            "procurement/procurement.html",
            user_name=session.get('User_Name'),
            role=session.get('User_Type'),
            sidebar_menu=get_menu('procurement', role=session.get('User_Type'),
                                  is_admin=_is_admin),
            active_item='mqsd',
            sidebar_extra_html=_PROC_SIDEBAR_EXTRA,
        )
    ...
    def api_suppliers_save():
        ...
        fields = {
            ...
            "state":           _s("state"),  # for GST inter-state detection
        }
        try:
            conn = sampling_portal.get_db_connection()
            if not conn:
    ...
    def api_po_pdf():
        ...
        try:
            ...
            if not o:
                return jsonify({"status": "error", "message": "PO not found"}), 404
            # ── Gate: PDF only available for approved POs ──
            if (o.get("approval_status") or "pending").lower() != "approved":
                conn.close()
                return jsonify({
                    "status": "error",
                    "code": "not_approved",
                    "message": "This PO has not been approved yet. Get it approved before printing or emailing."
                }), 403
            order = dict(o)
            ...
            order_discount = float(order.get("order_discount") or 0)
            # `total` is the items subtotal (sum of discounted line amounts).
            items_subtotal = total
            # Taxable base = items + freight + packing − order discount (never below zero)
            if order_discount > items_subtotal + freight + packing:
                order_discount = items_subtotal + freight + packing
            taxable_net = items_subtotal + freight + packing - order_discount
            if taxable_net < 0:
                taxable_net = 0
            # ── Inter-state detection ──
            # Compare company billing state vs supplier state. Different → IGST;
            # same → CGST + SGST. If either is missing, fall back to intra-state
            # as a safe default (frontend already warns users to set state).
            # IMPORTANT: read company state from the SAME source the live PO form
            # uses — procurement_settings.billing_state — so the printed PDF and the
            # on-screen totals always agree on IGST vs CGST/SGST. Fall back to the
            # billing godown's state only if the setting is empty.
            company_state = ""
            try:
                _cconn = sampling_portal.get_db_connection()
                if _cconn:
                    _cs = _cconn.execute(
                        "SELECT setting_value FROM procurement_settings WHERE setting_key='billing_state'"
                    ).fetchone()
                    if _cs:
                        company_state = (_cs["setting_value"] or "").strip().lower()
                    _cconn.close()
            except Exception:
                company_state = ""
            if not company_state and bill_godown:
                company_state = (dict(bill_godown).get("state") or "").strip().lower()
            supplier_state = (sup.get("state") or "").strip().lower() if sup else ""
            # If supplier state is blank but a GSTIN exists, derive it (first 2 digits)
            if not supplier_state and sup and sup.get("gst_number"):
                supplier_state = (_state_from_gstin(sup.get("gst_number")) or "").strip().lower()
            is_inter_state = bool(company_state and supplier_state and (company_state != supplier_state))

            # GST is charged on (items + freight + packing − discount). Freight,
            # packing and the order discount are apportioned across the item lines
            # in proportion to each line's amount, so each line is taxed at its rate.
            add_base = freight + packing - order_discount
            cgst = sgst = igst = 0
            for it in order["items"]:
                gp = float(it.get("gst_rate") or 0)
                # Defensive: stored as fraction means 0.18 instead of 18; normalize
                if 0 < gp < 1:
                    gp *= 100
                line_amt = float(it.get("amount") or 0)
                base = line_amt + (add_base * (line_amt / items_subtotal) if items_subtotal > 0 else 0)
                if gp > 0 and base > 0:
                    if is_inter_state:
                        igst += round(base * gp / 100, 2)
                    else:
                        c = round(base * (gp/2) / 100, 2)
                        cgst += c; sgst += c
            grand = taxable_net + cgst + sgst + igst
            ...
            for i, it in enumerate(order["items"], 1):
                ...
                amt  = float(it.get("amount") or 0) or (qty * rate * (1 - disc / 100.0))
                disc_display = ('{:g}'.format(disc) + "%") if disc else "—"
                gst  = it.get("gst_rate")
                # Defensive: if stored as fraction (e.g. 0.18) treat as 18%
                try:
                    _gv = float(gst) if gst is not None else None
                    if _gv is not None and 0 < _gv < 1:
                        _gv *= 100
                    gst_display = ('{:g}'.format(_gv) + "%") if _gv else "—"
                except (TypeError, ValueError):
                    gst_display = "—"
                item_rows += f"""<tr>
                    ...
                    <td style="padding:7px 9px;text-align:center;font-family:monospace">{disc_display}</td>
                    <td style="padding:7px 9px;text-align:center;font-family:monospace">{gst_display}</td>
            ...
            *{{box-sizing:border-box;margin:0;padding:0;-webkit-print-color-adjust:exact;print-color-adjust:exact}}
            ...
            .co{{font-size:22px;font-weight:900;color:#000;text-transform:uppercase;letter-spacing:.5px}}
            .cosub{{font-size:12px;font-weight:700;color:#000;margin-top:3px}}
            .pstat{{font-size:12px;font-weight:900;color:#000;padding:3px 10px;border:2px solid #000;border-radius:4px}}
            ...
            .ab{{padding:6px 9px;border-right:1px solid #000;font-size:10.5px;line-height:1.5;color:#000}}.ab:last-child{{border-right:none}}
            ...
            tbody td,tbody th{{border-left:1px solid #000}}
            ...
            <tfoot>
              <tr class="ftrow"><td colspan="6" style="text-align:right;color:#000;font-weight:700">Items Subtotal</td><td style="text-align:right;font-family:monospace">{fi(items_subtotal)}</td></tr>
              {'<tr class="ftrow"><td colspan="6" style="text-align:right;color:#000;font-weight:700">Freight Charges</td><td style="text-align:right;font-family:monospace">'+fi(freight)+'</td></tr>' if freight else ''}
              {'<tr class="ftrow"><td colspan="6" style="text-align:right;color:#000;font-weight:700">Packing Charges</td><td style="text-align:right;font-family:monospace">'+fi(packing)+'</td></tr>' if packing else ''}
              {'<tr class="ftrow"><td colspan="6" style="text-align:right;color:#000;font-weight:700">Discount</td><td style="text-align:right;font-family:monospace">- '+fi(order_discount)+'</td></tr>' if order_discount else ''}
              <tr class="ftrow"><td colspan="6" style="text-align:right;color:#000;font-weight:700">Taxable Amount</td><td style="text-align:right;font-family:monospace">{fi(taxable_net)}</td></tr>
              {'<tr class="ftrow"><td colspan="6" style="text-align:right;color:#000;font-weight:700">CGST</td><td style="text-align:right;font-family:monospace">'+fi(cgst)+'</td></tr>' if cgst else ''}
              {'<tr class="ftrow"><td colspan="6" style="text-align:right;color:#000;font-weight:700">SGST</td><td style="text-align:right;font-family:monospace">'+fi(sgst)+'</td></tr>' if sgst else ''}
              {'<tr class="ftrow"><td colspan="6" style="text-align:right;color:#000;font-weight:700">IGST</td><td style="text-align:right;font-family:monospace">'+fi(igst)+'</td></tr>' if igst else ''}
              <tr class="ftrow-total"><td colspan="6" style="text-align:right">Total</td><td style="text-align:right;font-family:monospace;color:#000;font-weight:900;font-size:15px">{fi(grand)}</td></tr>
            </tfoot></table>
            <div class="sig">
              <div class="sb"></div>
              <div class="sb" style="text-align:right"><div style="font-size:9px;font-weight:900;color:#000;text-transform:uppercase;letter-spacing:.6px;margin-bottom:4px">for HCP Wellness Pvt Ltd</div>
              <br><div style="font-size:11px;font-weight:700;color:#000;margin-top:16px">Authorised Signatory</div></div>
            </div>
    ...
    def api_po_save():
        ...
        try:
            ...
            packing_charge = None
        try:
            order_discount = float(d.get("order_discount") or 0) or None
        except (TypeError, ValueError):
            order_discount = None
        items    = d.get("items") or []   # [{material, qty, rate, discount}]
        uid      = session.get("UID", "")
        ...
        # Compute grand total (discounted line amounts + freight + packing + GST, minus order discount)
        # Taxable base = sum of per-line amounts (already net of per-line discount), then
        # the order-level discount is subtracted. GST is computed on this net base per-line.
        grand_total = 0.0
        _items_taxable = 0.0
        for item in items:
            try:
                ...
                line_amt = qty * rate * (1 - disc / 100.0)
                _items_taxable += line_amt
            except (TypeError, ValueError):
                pass
        # Subtract order-level discount from the taxable base (never below zero)
        taxable_base = _items_taxable - (order_discount or 0)
        if taxable_base < 0:
            taxable_base = 0
        grand_total = taxable_base + (freight_charge or 0) + (packing_charge or 0)
        ...
        try:
            ...
            if po_id:
                # ── UPDATE existing PO ──────────────────────────────────────
                # Editing a PO sends it back to 'pending' approval — re-approval is required
                conn.execute("""
                    UPDATE procurement_purchase_orders
                    SET po_num=%s, supplier_name=%s, status=%s, po_date=%s,
                        ...
                        freight_charge=%s, packing_charge=%s, order_discount=%s,
                ...
                """, (po_num, supplier, status, po_date, delivery, delivery_days,
                      ...
                      freight_charge, packing_charge, order_discount, voucher_type_name, uid, po_id))
                ...
                conn.execute("""
                    INSERT INTO procurement_purchase_orders
                        (po_num, supplier_name, status, po_date, delivery_date,
                         ...
                         tc_list_id, declaration_id, freight_charge, packing_charge, order_discount,
                         voucher_type_name, created_by, updated_by)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                """, (po_num, supplier, status, po_date, delivery, delivery_days,
                      remarks, grand_total, tc_list_id, declaration_id,
                      freight_charge, packing_charge, order_discount, voucher_type_name, uid, uid))
            ...
            for item in items:
                ...
                try:
                    ...
                    if disc < 0: disc = 0
                    if disc > 100: disc = 100
                    amount = ((qty or 0) * (rate or 0) * (1 - disc / 100.0)) or None
                    disc   = disc or None
                except (TypeError, ValueError):
                    qty = rate = amount = disc = None
                ...
                try:
                    ...
                    igst_amount = float(item.get("igst_amount") or 0) or None
                except (TypeError, ValueError):
                    cgst_amount = sgst_amount = igst_amount = None
                conn.execute("""
                    INSERT INTO procurement_po_items
                        (po_id, material, qty, qty_per_pkg, rate, amount, hsn_code, gst_rate, cgst_amount, sgst_amount, igst_amount, discount, packages, uom)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                """, (po_id, mat, qty,
                         float(item.get("qty_per_pkg") or 0) or None,
                         rate, amount, hsn_code, gst_rate, cgst_amount, sgst_amount, igst_amount, disc,
    ...
    @app.route("/api/procurement/po/bulk_import", methods=["POST"])
    @procurement_required
    def api_po_bulk_import():
        """
        Bulk-create POs from an Excel-parsed payload.
        ...
        Body:
        {
            "pos": [
                {
                    "po_date": "2026-06-01",
                    "supplier_name": "AHMEDABAD AGENCY",
                    "status": "open",
                    "delivery_date": "2026-06-15",
                    "remarks": "...",
                    "voucher_type_name": "General Purchase Order (General PO)",
                    "items": [
                        {"material":"Galaxy 226","qty":250,"qty_per_pkg":50,
                         "packages":5,"rate":135,"uom":"KG","hsn_code":"","gst_rate":18},
                        ...
                    ]
                },
                ...
        ...
        Each PO is processed in its own transaction so failures don't
        cascade. Returns {status, created, failed, results:[{ok, po_num,
        supplier_name, line_count, error}]}.
        """
        import re as _re
        d = request.get_json() or {}
        pos = d.get("pos") or []
        ...
        if not pos:
            return jsonify({"status": "error", "message": "no POs in payload"}), 400

        # ── Helper: atomically assign next PO number (mirrors api_po_save) ──
        def _bulk_assign_po_number(conn, voucher_type_name):
            today_str = __import__('datetime').date.today().isoformat()
        ...
        # ── Look up company state once (for inter-state GST detection) ──
        company_state = None
        try:
            _row = sampling_portal.get_db_connection().execute(
                "SELECT setting_value FROM procurement_settings WHERE setting_key='billing_state'"
            ).fetchone()
            if _row:
                company_state = (_row["setting_value"] or "").strip().lower() or None
        except Exception:
            pass

        for raw in pos:
            supplier = (raw.get("supplier_name") or "").strip()
            items    = raw.get("items") or []
            group_key = (raw.get("_group_key") or "").strip() or None
            entry    = {"ok": False, "po_num": None, "supplier_name": supplier,
                        "line_count": len(items), "error": None,
                        "group_key": group_key}

            if not supplier:
                entry["error"] = "supplier_name required"
                results.append(entry); failed += 1; continue
            if not items:
                entry["error"] = "at least one line item required"
                results.append(entry); failed += 1; continue

            status   = (raw.get("status") or "open").strip().lower()
            po_date  = raw.get("po_date") or None
            delivery = raw.get("delivery_date") or None
            remarks  = (raw.get("remarks") or "").strip() or None
            voucher_type_name = (raw.get("voucher_type_name") or "").strip() or None

            # Compute grand total (sum of qty*rate; no freight/packing in bulk)
            grand_total = 0.0
            for it in items:
                try:
                    grand_total += (float(it.get("qty") or 0) * float(it.get("rate") or 0))
                except (TypeError, ValueError):
                    pass
            ...
            try:
                ...
                po_num = _bulk_assign_po_number(conn, voucher_type_name)

                conn.execute("""
                    INSERT INTO procurement_purchase_orders
                        (po_num, supplier_name, status, po_date, delivery_date,
                         remarks, grand_total, voucher_type_name,
                         created_by, updated_by)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                """, (po_num, supplier, status, po_date, delivery, remarks,
                      grand_total, voucher_type_name, uid, uid))
                po_id = conn.execute("SELECT LAST_INSERT_ID() AS id").fetchone()["id"]

                # ── Determine inter-state vs intra-state for this PO ──
                # If either state is missing, fall back to intra-state (CGST/SGST)
                # as a conservative default — the frontend warns users to set
                # the state before they ever reach this code path.
                supplier_state = None
                try:
                    _sr = conn.execute(
                        "SELECT state FROM procurement_suppliers WHERE LOWER(TRIM(supplier_name))=LOWER(TRIM(%s)) LIMIT 1",
                        (supplier,)
                    ).fetchone()
                    if _sr:
                        supplier_state = (_sr["state"] or "").strip().lower() or None
                except Exception:
                    pass
                is_inter_state = bool(company_state and supplier_state and (company_state != supplier_state))
                ...
                for item in items:
                    ...
                    try:
                        ...
                        gst_rate = None
                    # Defensive: if frontend somehow sends a fraction (e.g. 0.18
                    # instead of 18) — almost always a user-entry mistake — convert
                    # it to its percentage form so downstream amounts stay correct.
                    if gst_rate is not None and 0 < gst_rate < 1:
                        gst_rate = gst_rate * 100
                    # Derive CGST/SGST or IGST based on inter-state flag
                    cgst_amount = sgst_amount = igst_amount = None
                    if gst_rate and amount:
                        if is_inter_state:
                            igst_amount = round(amount * gst_rate / 100.0, 4)
                        else:
                            half = round(amount * (gst_rate/2) / 100.0, 4)
                            cgst_amount = half
                            sgst_amount = half
                    conn.execute("""
                        INSERT INTO procurement_po_items
                            (po_id, material, qty, qty_per_pkg, rate, amount,
                             hsn_code, gst_rate, cgst_amount, sgst_amount, igst_amount, packages, uom)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                    """, (po_id, mat, qty,
                          float(item.get("qty_per_pkg") or 0) or None,
                          rate, amount, hsn_code, gst_rate, cgst_amount, sgst_amount, igst_amount,
                          int(item.get("packages")) if item.get("packages") else None,
                          (item.get("uom") or "KG").strip().upper() or None))
                    line_count += 1

                    # Write back rate to material master for future auto-fill
                    if rate:
                        try:
                            conn.execute("""
                                UPDATE procurement_materials
    ...
    @app.route("/api/procurement/po/can_approve", methods=["GET"])
    @procurement_required
    def api_po_can_approve():
        """Returns whether the current user can approve POs (used to show/hide UI buttons)."""
        return jsonify({
            "status": "ok",
            "can_approve": _can_approve_po(),
            "is_admin": _is_admin_user(),
            "username": _current_username()
        })

    @app.route("/api/procurement/po/approve", methods=["POST"])
    @procurement_required
    def api_po_approve():
        if not _can_approve_po():
            return jsonify({"status": "error", "message": "You are not authorized to approve POs"}), 403
        d = request.get_json() or {}
        po_id = d.get("id")
        if not po_id:
            return jsonify({"status": "error", "message": "id required"}), 400
        ...
        try:
            ...
            if not conn:
                return jsonify({"status": "error", "message": "DB connection failed"}), 500
            row = conn.execute(
                "SELECT id, po_num, approval_status FROM procurement_purchase_orders WHERE id=%s",
                (po_id,)
            ).fetchone()
            if not row:
                conn.close()
                return jsonify({"status": "error", "message": "PO not found"}), 404
            conn.execute(
                "UPDATE procurement_purchase_orders SET approval_status='approved', approved_by=%s, approved_at=NOW() WHERE id=%s",
                (approver, po_id)
            )
            conn.commit(); conn.close()
            return jsonify({
    ...
    @app.route("/api/procurement/po/revoke_approval", methods=["POST"])
    @procurement_required
    def api_po_revoke_approval():
        """Admin (or an approver) can revoke an approval — sends the PO back to pending."""
        if not _can_approve_po():
        ...
        try:
            ...
            if not conn:
                return jsonify({"status": "error", "message": "DB connection failed"}), 500
            conn.execute(
                "UPDATE procurement_purchase_orders SET approval_status='pending', approved_by=NULL, approved_at=NULL WHERE id=%s",
                (po_id,)
            )
            conn.commit(); conn.close()
            return jsonify({"status": "ok", "approval_status": "pending"})
        except Exception as e:
            traceback.print_exc()
            return jsonify({"status": "error", "message": str(e)}), 500

    # ── Approvers management (admin-only) ─────────────────────────────────

    @app.route("/api/procurement/po_approvers/list", methods=["GET"])
    @procurement_required
    def api_po_approvers_list():
        """List approvers + all users. Admin-only (UI also gates it but server enforces)."""
        ...
        try:
            ...
            if not conn:
                return jsonify({"status": "error", "message": "DB connection failed"}), 500
            # Get all users
            users = conn.execute("SELECT id, username, full_name, email, user_type, is_active FROM User_Tbl ORDER BY username").fetchall()
            users_list = [dict(u) for u in users]
            # Get current approvers
            approvers = conn.execute("SELECT username, granted_by, granted_at FROM procurement_po_approvers").fetchall()
            approver_set = {(a["username"] or "").lower(): dict(a) for a in approvers}
            for u in users_list:
                key = (u.get("username") or "").lower()
                u["is_approver"] = key in approver_set
                u["granted_by"]  = approver_set.get(key, {}).get("granted_by")
                u["granted_at"]  = str(approver_set.get(key, {}).get("granted_at") or "")
                # The literal 'admin' user always has approval rights — UI shows this as built-in
                u["is_admin_user"] = (key == "admin")
            conn.close()
    ...
    @app.route("/api/procurement/po_approvers/set", methods=["POST"])
    @procurement_required
    def api_po_approvers_set():
        """Admin grants or revokes approval rights for a user."""
        if not _is_admin_user():
            return jsonify({"status": "error", "message": "Admin only"}), 403
        d = request.get_json() or {}
        target = (d.get("username") or "").strip().lower()
        grant  = bool(d.get("grant"))
        if not target:
            return jsonify({"status": "error", "message": "username required"}), 400
        if target == "admin":
            return jsonify({"status": "error", "message": "The 'admin' user always has approval rights"}), 400
        try:
            conn = sampling_portal.get_db_connection()
            if not conn:
                return jsonify({"status": "error", "message": "DB connection failed"}), 500
            if grant:
                # Verify the username actually exists
                row = conn.execute("SELECT username FROM User_Tbl WHERE LOWER(TRIM(username))=%s LIMIT 1", (target,)).fetchone()
                if not row:
                    conn.close()
                    return jsonify({"status": "error", "message": f"User '{target}' not found"}), 404
                conn.execute(
                    "INSERT INTO procurement_po_approvers (username, granted_by) VALUES (%s,%s) "
                    "ON DUPLICATE KEY UPDATE granted_by=VALUES(granted_by), granted_at=CURRENT_TIMESTAMP",
                    (target, _current_username())
                )
            else:
                conn.execute("DELETE FROM procurement_po_approvers WHERE LOWER(TRIM(username))=%s", (target,))
            conn.commit(); conn.close()
            return jsonify({"status": "ok", "username": target, "is_approver": grant})
        except Exception as e:
            traceback.print_exc()
            return jsonify({"status": "error", "message": str(e)}), 500


    # ── User profile (Update Profile dropdown) ────────────────────────────

    @app.route("/api/user/profile", methods=["GET"])
    @procurement_required
    def api_user_profile_get():
        """Return the current user's profile fields (email + mobile) for the modal."""
        uname = _current_username()
        if not uname:
        ...
        try:
            ...
            if not conn:
                return jsonify({"status": "error", "message": "DB connection failed"}), 500
            row = conn.execute(
                "SELECT id, username, full_name, email, mobile, user_type FROM User_Tbl WHERE LOWER(TRIM(username))=%s LIMIT 1",
                (uname,)
            ).fetchone()
            conn.close()
            if not row:
                return jsonify({"status": "error", "message": "User not found"}), 404
            return jsonify({"status": "ok", "profile": dict(row)})
        except Exception as e:
            traceback.print_exc()
            return jsonify({"status": "error", "message": str(e)}), 500
    ...
    def api_user_profile_save():
        """Update the current user's email, mobile, and (optionally) password.

        Body: { email, mobile, password }
        - email/mobile are always set (empty string → NULL)
        - password is only updated when provided AND non-empty
        - Passwords are stored as werkzeug PBKDF2 hash (matches Flask convention)
        """
        uname = _current_username()
        if not uname:
            return jsonify({"status": "error", "message": "Not logged in"}), 401
        ...
        password = d.get("password") or ""   # raw plaintext, only used if non-empty
        # Basic validation
        ...
        try:
            ...
            if not conn:
                return jsonify({"status": "error", "message": "DB connection failed"}), 500
            # Verify the user exists before update (defensive)
            row = conn.execute(
                "SELECT id FROM User_Tbl WHERE LOWER(TRIM(username))=%s LIMIT 1",
                (uname,)
            ).fetchone()
            if not row:
                conn.close()
            ...
            if password:
                ...
                conn.execute(
                    "UPDATE User_Tbl SET email=%s, mobile=%s, password_hash=%s WHERE id=%s",
                    (email or None, mobile or None, pw_hash, row["id"])
                )
            else:
                conn.execute(
                    "UPDATE User_Tbl SET email=%s, mobile=%s WHERE id=%s",
                    (email or None, mobile or None, row["id"])
                )
            conn.commit(); conn.close()
            return jsonify({"status": "ok", "username": uname, "password_changed": bool(password)})
        except Exception as e:
            traceback.print_exc()
            return jsonify({"status": "error", "message": str(e)}), 500


    # ── Procurement-specific user email settings (Gmail SMTP + defaults) ──

    @app.route("/api/procurement/user/email_settings", methods=["GET"])
    @procurement_required
    def api_user_email_settings_get():
        """Return the current user's procurement email settings.
        Empty row is fine — frontend handles defaults."""
        uname = _current_username()
        if not uname:
            return jsonify({"status": "error", "message": "Not logged in"}), 401
        try:
            ...
            if not conn:
                return jsonify({"status": "error", "message": "DB connection failed"}), 500
            row = conn.execute(
                "SELECT * FROM procurement_user_email_settings WHERE LOWER(TRIM(username))=%s LIMIT 1",
                (uname,)
            ).fetchone()
            conn.close()
            if row:
                settings = dict(row)
                # Don't expose the actual app password — only whether one is set.
                # This keeps the password from leaking back to the client in the GET response.
                # The frontend uses 'has_password' to show "Saved (click to change)" vs "Not set".
                settings["has_password"] = bool(settings.get("gmail_app_password"))
                settings["gmail_app_password"] = ""
                settings.pop("password_encrypted", None)  # internal-only flag; don't expose
                if settings.get("updated_at"): settings["updated_at"] = str(settings["updated_at"])
                # Default the WhatsApp template if user hasn't set one yet
                if not settings.get("whatsapp_default_msg"):
                    settings["whatsapp_default_msg"] = (
                        "Hello {contact_person},\n\n"
                        "Please find attached our Purchase Order {po_num} dated {po_date}.\n\n"
                        "Kindly acknowledge and confirm delivery timeline.\n\n"
                        "Regards"
                    )
            else:
                settings = {
                    ...
                    "gmail_app_password": "",
                    "email_default_cc": "",
                    "email_default_subject": "RMPO | {supplier_name} | {po_num}",
                    "email_default_body": (
                        "Dear {contact_person},\n\n"
                        "Please find attached our Purchase Order {po_num} dated {po_date}.\n\n"
    ...
    @app.route("/api/procurement/user/email_settings", methods=["POST"])
    @procurement_required
    def api_user_email_settings_save():
        """Upsert the current user's procurement email settings.
        Body: { gmail_address, gmail_app_password, email_default_cc,
                email_default_subject, email_default_body, email_signature }
        If gmail_app_password is empty string, the EXISTING password is preserved.
        Pass an explicit '__CLEAR__' to wipe the stored password.
        """
        uname = _current_username()
        if not uname:
            return jsonify({"status": "error", "message": "Not logged in"}), 401
        d = request.get_json() or {}
        gmail_addr = (d.get("gmail_address") or "").strip()
        app_pw_raw = d.get("gmail_app_password") or ""
        cc_default = (d.get("email_default_cc") or "").strip()
        subj_def   = (d.get("email_default_subject") or "").strip()
        body_def   = (d.get("email_default_body") or "")
        signature  = (d.get("email_signature") or "")
        wa_default = (d.get("whatsapp_default_msg") or "")
        # Validate email
        if gmail_addr and "@" not in gmail_addr:
            return jsonify({"status": "error", "message": "Invalid Gmail address"}), 400
        try:
            conn = sampling_portal.get_db_connection()
            if not conn:
                return jsonify({"status": "error", "message": "DB connection failed"}), 500
            # Decide whether to update the app password column
            if app_pw_raw == "__CLEAR__":
                pw_to_store = None
                pw_encrypted_flag = 0
                pw_changed  = True
            elif app_pw_raw.strip() == "":
                # Preserve existing
                pw_to_store = None
                pw_encrypted_flag = None  # don't touch flag either
                pw_changed  = False
            else:
                # New password — strip whitespace (Gmail app passwords have no spaces)
                raw_pw = app_pw_raw.replace(" ", "").strip()
                pw_to_store, was_encrypted = _encrypt_secret(raw_pw)
                pw_encrypted_flag = 1 if was_encrypted else 0
                pw_changed  = True
            # Upsert — common (non-password) columns first
            if pw_changed:
                conn.execute("""
                    INSERT INTO procurement_user_email_settings
                        (username, gmail_address, gmail_app_password, password_encrypted,
                         email_default_cc, email_default_subject, email_default_body,
                         email_signature, whatsapp_default_msg)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
                    ON DUPLICATE KEY UPDATE
                        gmail_address=VALUES(gmail_address),
                        gmail_app_password=VALUES(gmail_app_password),
                        password_encrypted=VALUES(password_encrypted),
                        email_default_cc=VALUES(email_default_cc),
                        email_default_subject=VALUES(email_default_subject),
                        email_default_body=VALUES(email_default_body),
                        email_signature=VALUES(email_signature),
                        whatsapp_default_msg=VALUES(whatsapp_default_msg)
                """, (uname, gmail_addr or None, pw_to_store, pw_encrypted_flag,
                      cc_default or None, subj_def or None, body_def or None,
                      signature or None, wa_default or None))
            else:
                # Don't overwrite the existing password
                conn.execute("""
                    INSERT INTO procurement_user_email_settings
                        (username, gmail_address, email_default_cc,
                         email_default_subject, email_default_body, email_signature,
                         whatsapp_default_msg)
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        email_signature=VALUES(email_signature),
                        whatsapp_default_msg=VALUES(whatsapp_default_msg)
                """, (uname, gmail_addr or None,
                      cc_default or None, subj_def or None, body_def or None,
                      signature or None, wa_default or None))
            conn.commit(); conn.close()
            return jsonify({"status": "ok", "password_changed": pw_changed})
        except Exception as e:
            traceback.print_exc()
            return jsonify({"status": "error", "message": str(e)}), 500


    # ── Send PO via Gmail SMTP (uses sender's own credentials) ────────────

    @app.route("/api/procurement/po/email", methods=["POST"])
    @procurement_required
    def api_po_email():
        """Send a PO PDF via Gmail SMTP using the current user's credentials.
        Body: { po_id, to, cc, subject, body }
        Returns 403 if PO is unapproved (same gate as PDF endpoint).
        Returns 412 (precondition failed) if user has no Gmail creds saved.
        """
        import smtplib, ssl
        ...
        d = request.get_json() or {}
        po_id   = d.get("po_id") or d.get("id")
        to_addr = (d.get("to") or "").strip()
        cc_addr = (d.get("cc") or "").strip()
        subject = (d.get("subject") or "").strip()
        body    = (d.get("body") or "")

        if not po_id:
            return jsonify({"status": "error", "message": "po_id required"}), 400
        if not to_addr:
        ...
        try:
            ...
            # 1) Load user's email settings
            settings = conn.execute(
                "SELECT gmail_address, gmail_app_password, password_encrypted FROM procurement_user_email_settings WHERE LOWER(TRIM(username))=%s LIMIT 1",
                (uname,)
            ).fetchone()
            if not settings or not settings.get("gmail_address") or not settings.get("gmail_app_password"):
                conn.close()
                return jsonify({
                    "status": "error",
                    "code": "no_gmail_creds",
                    "message": "Gmail credentials not set. Open Update Profile → Email Settings to configure your Gmail address and app password."
                }), 412

            gmail_addr = settings["gmail_address"].strip()
            # Decrypt password if it was stored encrypted; otherwise treat as legacy plaintext
            _is_enc   = bool(settings.get("password_encrypted"))
            gmail_pw  = _decrypt_secret(settings["gmail_app_password"], _is_enc).strip()
            if not gmail_pw:
                conn.close()
                return jsonify({
                    "status": "error",
                    "code": "decrypt_failed",
                    "message": "Could not read your stored Gmail credentials. Please re-save them in Update Profile."
                }), 500

            # 2) Verify PO exists AND is approved (re-uses the PDF gating logic)
            po_row = conn.execute(
                "SELECT id, po_num, approval_status FROM procurement_purchase_orders WHERE id=%s",
                (po_id,)
            ).fetchone()
            if not po_row:
                conn.close()
                return jsonify({"status": "error", "message": "PO not found"}), 404
            if (po_row.get("approval_status") or "pending").lower() != "approved":
                conn.close()
                return jsonify({
                    "status": "error",
                    "code": "not_approved",
                    "message": "This PO has not been approved yet. Get it approved before emailing."
                }), 403

            po_num = po_row["po_num"] or f"PO-{po_id}"
            conn.close()

            # 3) Generate PDF in-memory by calling the same view function used for downloads.
            # We invoke the existing /api/procurement/po/pdf logic via test client so we don't
            # duplicate the PDF-building code. Approval was already verified above.
            with app.test_client() as client:
                # Forward session via Flask's session cookie (test client shares the session).
                pdf_resp = client.get(f"/api/procurement/po/pdf?id={po_id}",
                                      headers={"Cookie": request.headers.get("Cookie", "")})
                if pdf_resp.status_code != 200:
                    return jsonify({"status": "error", "message": f"PDF generation failed (status {pdf_resp.status_code})"}), 500
                pdf_bytes = pdf_resp.data

            # 4) Build MIME message
            msg = MIMEMultipart()
            msg["From"]     = formataddr((session.get("UID", "Procurement"), gmail_addr))
            msg["To"]       = to_addr
            if cc_addr:
                msg["Cc"]   = cc_addr
            msg["Reply-To"] = gmail_addr
            msg["Subject"]  = subject
            msg["Message-ID"] = make_msgid(domain="hcpwellness.in")

            # Body as plain text (preserves line breaks the user typed)
            msg.attach(MIMEText(body, "plain", "utf-8"))

            # PDF attachment
            safe_name = (po_num or f"PO-{po_id}").replace("/", "_").replace("\\", "_")
            pdf_part = MIMEApplication(pdf_bytes, _subtype="pdf")
            pdf_part.add_header("Content-Disposition", "attachment", filename=f"PO_{safe_name}.pdf")
            msg.attach(pdf_part)

            # 5) Send via Gmail SMTP
            recipients = [to_addr]
            if cc_addr:
                # Allow comma- or semicolon-separated cc lists
                for c in cc_addr.replace(";", ",").split(","):
                    c = c.strip()
                    if c: recipients.append(c)
            # Small helper: log every send attempt (success or failure) to the audit table.
            # Logging is best-effort — never fail the user's request because logging failed.
            def _log_email_send(status_str, err_msg=None):
                try:
                    _ec = sampling_portal.get_db_connection()
                    if _ec:
                        _ec.execute(
                            """INSERT INTO procurement_po_email_log
                                (po_id, po_num, sent_by, to_addr, cc_addr, subject, status, error_msg)
                               VALUES (%s, %s, %s, %s, %s, %s, %s, %s)""",
                            (po_id, po_num, uname, to_addr, cc_addr or None,
                             subject, status_str, err_msg)
                        )
                        _ec.commit(); _ec.close()
                except Exception:
                    pass  # silent — audit logging must never break the actual send

            try:
                ssl_ctx = ssl.create_default_context()
                with smtplib.SMTP_SSL("smtp.gmail.com", 465, context=ssl_ctx, timeout=30) as smtp:
                    smtp.login(gmail_addr, gmail_pw)
                    smtp.send_message(msg, from_addr=gmail_addr, to_addrs=recipients)
            except smtplib.SMTPAuthenticationError as auth_err:
                _log_email_send("failed", f"SMTP auth failed: {str(auth_err)[:200]}")
                return jsonify({
                    "status": "error",
                    "code": "smtp_auth",
                    "message": "Gmail authentication failed. Check your Gmail address and app password. "
                               "Make sure 2-Step Verification is enabled and you're using an App Password, not your regular Gmail password."
                }), 502
            except smtplib.SMTPException as smtp_err:
                _log_email_send("failed", f"SMTP error: {str(smtp_err)[:200]}")
                return jsonify({
                    "status": "error",
                    "code": "smtp_error",
                    "message": f"Gmail SMTP error: {str(smtp_err)}"
                }), 502
            except Exception as net_err:
                _log_email_send("failed", f"Network: {str(net_err)[:200]}")
                return jsonify({
                    "status": "error",
                    "code": "network",
                    "message": f"Could not connect to Gmail SMTP: {str(net_err)}"
                }), 502

            # Successful send
            _log_email_send("sent")
    ...
    # ── Email send audit log (admin only) ─────────────────────────────────

    @app.route("/api/procurement/po/email_log", methods=["GET"])
    @procurement_required
    def api_po_email_log():
        """Admin-only: return the most recent N email-send attempts (success + failure).
        Query params: ?limit=100 (max 500), ?po_id=N (filter by specific PO)
        """
        if not _is_admin_user():
            return jsonify({"status": "error", "message": "Admin only"}), 403
        try:
        ...
        po_id_filter = request.args.get("po_id")
        try:
            conn = sampling_portal.get_db_connection()
            if not conn:
                return jsonify({"status": "error", "message": "DB connection failed"}), 500
            if po_id_filter:
                rows = conn.execute(
                    "SELECT * FROM procurement_po_email_log WHERE po_id=%s ORDER BY sent_at DESC LIMIT %s",
                    (po_id_filter, limit)
                ).fetchall()
            else:
                rows = conn.execute(
                    "SELECT * FROM procurement_po_email_log ORDER BY sent_at DESC LIMIT %s",
                    (limit,)
                ).fetchall()
            conn.close()
            out = []
            for r in rows:
                ...
                out.append(d)
            return jsonify({"status": "ok", "log": out, "count": len(out)})
        except Exception as e:
            traceback.print_exc()
            return jsonify({"status": "error", "message": str(e)}), 500
