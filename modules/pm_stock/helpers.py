...
def _heal_box_location(conn, box_id, expected_godown_id, current_godown_id, user=None):
    ...
    # cheaper than ordering by movement_at.
    #
    # NOTE: the timestamp column on pm_box_movements is `movement_at`,
    # not `created_at`. Selecting `created_at` here used to crash the
    # whole scan_box flow with "Unknown column 'created_at' in 'field
    # list'" the moment a box's stored location had drifted and this
    # heal path was reached.
    last_mv = conn.execute(
        """SELECT movement_id, movement_type, from_godown_id, to_godown_id, movement_at
...
def _assert_grn_box_invariant(conn, grn_id):
    """Safety net: verify pm_boxes row-count matches sum of pm_grn_items.no_of_box
    for a given GRN. Raises RuntimeError on mismatch (caller's transaction will
    roll back when the exception propagates).

    Background:
    -----------
    Historically, two GRNs (PM-GRN/0312 and PM-GRN/0342, 26-27 financial year)
    ended up with "orphan" boxes — pm_boxes rows pointing to pm_grn_items.id
    values that had been deleted. This made the Verify GRN modal display
    double the actual box count (and 2x BOXES TOTAL QTY).

    The root cause was a GRN edit/update path that deleted the pm_grn_items
    rows without first deleting the matching pm_boxes rows (or vice versa,
    re-inserted items + boxes without first clearing the old boxes).

    This guard catches such corruption AT THE POINT of the bad write — before
    commit — so the operation fails cleanly with a clear error instead of
    silently leaving the DB in a half-broken state.

    Where to call:
    --------------
    Inside any endpoint that writes pm_grn_items or pm_boxes for an existing
    GRN, immediately before `conn.commit()`. On match, the function is silent
    and the commit proceeds. On mismatch, it raises and the outer try/except
    in the endpoint returns an error to the client; MySQL auto-rolls-back the
    transaction when the connection closes without commit.

    Why this is safe to deploy:
    ---------------------------
    - Zero behaviour change for healthy data: only fires on actual corruption.
    - Two trivial SELECTs, negligible performance cost.
    - Cannot itself corrupt anything — it only reads + raises.
    """
    row = conn.execute(
        """SELECT
             (SELECT COALESCE(SUM(no_of_box), 0)
                FROM pm_grn_items WHERE grn_id=%s) AS expected_boxes,
             (SELECT COUNT(*)
                FROM pm_boxes      WHERE grn_id=%s) AS actual_boxes""",
        (grn_id, grn_id)
    ).fetchone()
    expected = int((row['expected_boxes'] if row else 0) or 0)
    actual   = int((row['actual_boxes']   if row else 0) or 0)
    if expected != actual:
        # Include both numbers so the diagnostic is in the error log.
        raise RuntimeError(
            f"GRN box invariant violation (grn_id={grn_id}): "
            f"pm_grn_items.no_of_box sums to {expected} but pm_boxes has "
            f"{actual} rows. Refusing to commit — this would create orphan "
            f"or missing box rows. Operation rolled back; data is unchanged."
        )
