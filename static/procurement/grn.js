...
async function openGrnForm(row) {
    ...
    if (typeof comboboxAutoInit === 'function') comboboxAutoInit(fbody);
    ...
    _grnRefreshMatDatalist(); // populate material datalist after rows loaded

    // Load types first (needed for PO filter), then populate PO dropdown
    await grnEnsureTypesLoaded();
    ...
    if (row && row.id) {
        ...
        try {
            ...
            await grnLoadVoucherTypes(o.voucher_type_name || '');
            ...
            _grnLines = (o.items||[]).map(function(i){
                return {
                    ...
                    uom:          i.uom          || ((_allRows||[]).find(function(r){ return (r.material_name||'').toLowerCase()===(i.material||'').toLowerCase(); })||{}).uom || 'KG',
                    packages:     i.packages     || '',
                    qty_per_pkg:  i.qty_per_pkg  || i.received_qty || '',
                    total_qty:    parseFloat(i.received_qty) || 0
                };
            });
    ...
    if (!_grnLines.length) _grnLines.push({material:'',po_qty:'',received_qty:'',qty_per_pkg:'',packages:'',total_qty:0,rate:'',hsn_code:'',gst_rate:0,location:'',invoice_num:'',invoice_date:'',batch_num:'',mfg_date:'',expiry_date:'',uom:'KG'});
    grnRenderLines();
...
/* ══════════════════════════════════════════════
   GRN VOUCHER TYPE — loads from GOP, populates dropdown
══════════════════════════════════════════════ */
async function grnLoadVoucherTypes(currentTypeName) {
    var sel = document.getElementById('grnVoucherType');
    if (!sel) return;
    sel.innerHTML = '<option value="">— Select Type —</option>';
    // Also load PO voucher types so we can match PO's voucher_type_name to mat_type_abbr
    if (!_grnPoTypeList.length) {
        try {
            var ptr = await fetch('/api/gop/voucher_types?parent_type=po');
            var ptd = await ptr.json();
            if (ptd.status === 'ok') _grnPoTypeList = ptd.types || [];
        } catch(e) {}
    }
    try {
        var res  = await fetch('/api/gop/voucher_types?parent_type=grn');
        var data = await res.json();
        if (data.status !== 'ok') return;
        _grnVoucherTypeList = (data.types || []).filter(function(t){ return t.is_active; });
        _grnVoucherTypeList.forEach(function(t) {
        ...
        // Auto-select first if nothing selected and creating new
        if (!currentTypeName && sel.options.length === 2) sel.selectedIndex = 1;
        if (typeof comboboxRefresh === 'function') comboboxRefresh(sel);
        // Trigger type change to set group filter
        grnVoucherTypeChange();
    } catch(e) { /* silently ignore — type dropdown is optional */ }
}

var _grnMatGroupFilter = null;

var _grnPrevVoucherType = '';  // track previously selected type

function grnVoucherTypeChange() {
    var sel      = document.getElementById('grnVoucherType');
    var typeName = sel ? sel.value : '';

    // ── Confirm reset if data already entered ──
    var hasData = _grnLines.some(function(l){ return (l.material||'').trim(); }) ||
                  _grnPoInvoices.length > 0;
    if (_grnPrevVoucherType && typeName !== _grnPrevVoucherType && hasData) {
        if (!confirm('Changing the GRN type will clear all current line items and PO links. Continue?')) {
            sel.value = _grnPrevVoucherType;  // revert
            return;
        }
        // Reset all data
        _grnLines      = [{material:'',po_qty:'',received_qty:'',qty_per_pkg:'',packages:'',total_qty:0,
                           rate:'',hsn_code:'',gst_rate:0,location:'',invoice_num:'',invoice_date:'',
                           batch_num:'',mfg_date:'',expiry_date:'',uom:'KG'}];
        _grnPoInvoices = [];
        var sup = document.getElementById('grnSupplier');
        if (sup) sup.value = '';
        grnRenderLines();
        grnRenderPoInvoices();
    }
    _grnPrevVoucherType = typeName;

    // ── Feature 1: Update GRN number preview ──
    if (typeName) {
        var numEl = document.getElementById('grnFormNum');
        if (numEl && (numEl.textContent === 'Auto-assigned on save' || !numEl.textContent.trim())) {
            fetch('/api/gop/voucher_numbering/next?voucher_type=' + encodeURIComponent(typeName))
                .then(function(r){ return r.json(); })
                .then(function(d){
    ...
    // ── Feature 2: Filter GRN material datalist by material type association ──
    if (typeof _grnVoucherTypeList !== 'undefined') {
        var typeInfo = (_grnVoucherTypeList||[]).find(function(t){ return t.name === typeName; });
        ...
        if      (matAbbr === 'RM') _grnMatGroupFilter = 'rm';
        else if (matAbbr === 'PM') _grnMatGroupFilter = 'pm';
        else if (matAbbr === 'FG') _grnMatGroupFilter = 'fg';
        else                       _grnMatGroupFilter = null;
    }
    _grnRefreshMatDatalist();
    // Refresh PO dropdown to match new type filter
    grnPopulatePOSelect('');
}

var _grnVoucherTypeList = [];  // GRN types
var _grnPoTypeList      = [];  // PO types

/* Ensure both GRN and PO voucher types are loaded before using them */
async function grnEnsureTypesLoaded() {
    if (_grnPoTypeList.length && _grnVoucherTypeList.length) return;
    if (!_grnPoTypeList.length) {
        try {
            var r = await fetch('/api/gop/voucher_types?parent_type=po');
            var d = await r.json();
            if (d.status === 'ok') _grnPoTypeList = d.types || [];
        } catch(e) {}
    }
    if (!_grnVoucherTypeList.length) {
        try {
            var r2 = await fetch('/api/gop/voucher_types?parent_type=grn');
            var d2 = await r2.json();
            if (d2.status === 'ok') _grnVoucherTypeList = (d2.types || []).filter(function(t){ return t.is_active; });
        } catch(e) {}
    }
}

function _grnRefreshMatDatalist() {
    var dl = document.getElementById('grnMatList');
    if (!dl || !_allRows) return;

    var allMatch = (_allRows || []).filter(function(r){
        if (!_grnMatGroupFilter) return true;
        var abbr = (r.mat_type_abbr || '').toUpperCase();
        if (abbr) {
            if (_grnMatGroupFilter === 'rm') return abbr === 'RM';
            if (_grnMatGroupFilter === 'pm') return abbr === 'PM';
            if (_grnMatGroupFilter === 'fg') return abbr === 'FG';
            return true;
        }
        // Fallback: group_name keyword
        var grp = (r.group_name || '').toLowerCase();
        if (_grnMatGroupFilter === 'rm') return grp.includes('raw');
        if (_grnMatGroupFilter === 'pm') return grp.includes('pack');
        if (_grnMatGroupFilter === 'fg') return grp.includes('finish') || grp.includes('fg');
        return true;
    });

    // Graceful fallback: if filter active but nothing matches, show all
    var showWarning = false;
    if (_grnMatGroupFilter && allMatch.length === 0) {
        allMatch = (_allRows || []).slice();
        showWarning = true;
    }

    dl.innerHTML = allMatch.map(function(r){
        return '<option value="' + escHtml(r.material_name || '') + '">';
    }).join('');

    // Dynamic badge next to GRN header
    _grnUpdateMatFilterBadge(showWarning);
}

function _grnUpdateMatFilterBadge(warn) {
    // Inject badge dynamically if not present
    var header = document.getElementById('grnFormNum');
    if (header && !document.getElementById('grnMatFilterBadge')) {
        var badge = document.createElement('span');
        badge.id = 'grnMatFilterBadge';
        badge.style.cssText = 'display:none;font-size:9.5px;font-weight:700;padding:2px 8px;border-radius:10px;margin-left:8px';
        header.parentNode.insertBefore(badge, header.nextSibling);
    }
    var ind = document.getElementById('grnMatFilterBadge');
    if (!ind) return;
    var filterLabel = { rm: 'RM only', pm: 'PM only', fg: 'FG only' };
    if (!_grnMatGroupFilter) { ind.style.display = 'none'; return; }
    ind.style.display = 'inline-flex';
    if (warn) {
        ind.textContent = '⚠ No types assigned — showing all';
        ind.style.background = 'rgba(251,191,36,.15)';
        ind.style.color = '#b45309';
    } else {
        ind.textContent = '🔵 ' + (filterLabel[_grnMatGroupFilter] || _grnMatGroupFilter.toUpperCase());
        ind.style.background = 'rgba(37,99,235,.1)';
        ind.style.color = '#1d4ed8';
    }
}

function grnCalcLineTotal(i) {
    var pkgsEl  = document.querySelector('#grnLinesTbody tr[data-gi="'+i+'"] .grn-pkgs-inp');
    var qppEl   = document.querySelector('#grnLinesTbody tr[data-gi="'+i+'"] .grn-qpp-inp');
    var totalEl = document.querySelector('#grnLinesTbody tr[data-gi="'+i+'"] .grn-total-cell');
    if (!pkgsEl || !qppEl) return;
    ...
    _grnLines[i].received_qty = total > 0 ? String(total) : _grnLines[i].received_qty;
    if (totalEl) {
        totalEl.textContent = total > 0 ? total.toLocaleString('en-IN',{maximumFractionDigits:3}) : '\u2014';
...
function grnPopulatePOSelect(selectedPoId) {
    ...
    var grnTypeInfo = (_grnVoucherTypeList||[]).find(function(t){ return t.name === grnTypeName; });
    var grnMatAbbr  = grnTypeInfo ? (grnTypeInfo.mat_type_abbr || '').toUpperCase() : '';

    // Symmetric match: GRN type's mat_abbr must equal PO type's mat_abbr.
    // General GRN (no mat_abbr) only sees General POs (no mat_abbr).
    // Lab GRN only sees Lab POs. RM GRN only sees RM POs. No cross-loading ever.
    var matched = [];
    (_poRows||[]).forEach(function(r) {
        if (r.status === 'cancelled' || r.status === 'closed') return;
        var poTypeInfo = (_grnPoTypeList||[]).find(function(t){ return t.name === (r.voucher_type_name||''); });
        var poMatAbbr  = poTypeInfo ? (poTypeInfo.mat_type_abbr || '').toUpperCase() : '';
        if (poMatAbbr === grnMatAbbr) matched.push(r);
    });

    var opts = '<option value="">— Manual (no linked PO) —</option>';
    if (matched.length === 0 && grnTypeName) {
        opts += '<option value="" disabled>— No open POs for ' + escHtml(grnTypeName) + ' —</option>';
    } else {
        matched.forEach(function(r) {
            opts += '<option value="'+r.id+'"'+(String(r.id)===String(selectedPoId)?' selected':'')+'>'+escHtml(r.po_num)+' – '+escHtml(r.supplier_name||'')+'</option>';
        });
    }
    sel.innerHTML = opts;
    if (typeof comboboxRefresh === 'function') comboboxRefresh(sel);
...
function grnPoChange() {
    ...
    if (!po) return;
    // Fill supplier — _poRows may come from po.js (field: 'supplier') or grn.js raw (field: 'supplier_name')
    var supEl = document.getElementById('grnSupplier');
    if (supEl) supEl.value = po.supplier_name || po.supplier || '';
    ...
    // Fetch full PO items with pending quantities
        ...
        .then(function(d){
            ...
            _grnLines = (o.items||[]).map(function(i){
                ...
                return {
                    ...
                    received_qty: pendingQty,  // pre-fill with pending (not full PO qty)
                    rate:         i.rate      != null ? parseFloat(i.rate)     : '',
                ...
                // Drop items that are fully received — nothing left to receive
...
function grnRenderLines() {
    ...
    if (!_grnLines.length) {
        tb.innerHTML = '<tr><td colspan="16" style="padding:24px;text-align:center;color:var(--muted);font-size:12px">No items — select a PO above or click "+ Add Item"</td></tr>';
        grnCalcTotal(); return;
    }
    tb.innerHTML = _grnLines.map(function(line, i){
        ...
        var pq          = line.po_qty !== '' ? parseFloat(line.po_qty) : null;
        ...
        // Display pending qty in cell; full PO qty + already received in tooltip
        var displayQty  = pendingQty !== null ? pendingQty : pq;
        var poQtyTitle  = pq != null
            ? 'PO Qty: ' + pq
            + (hasPartial ? ' | Already received: ' + alreadyRcvd + ' | Pending: ' + pendingQty : '')
            : '';
        // Amber = partial; green tint = full fresh PO; normal grey when no PO
        var poQtyStyle  = MS + ';opacity:1'
            + (hasPartial
                ? ';color:#b45309;border-color:rgba(217,119,6,.5);background:rgba(255,251,235,.7)'
                : (pq !== null ? '' : ';opacity:.5'));
        return '<tr data-gi="'+i+'" style="border-bottom:1px solid var(--border);background:'+bg+';">'
            ...
            +'<td style="padding:3px 4px;min-width:160px;vertical-align:top"><input class="grn-li-inp grn-mat-inp" list="grnMatList" data-gi="'+i+'" value="'+escHtml(line.material||'')+'" placeholder="Material…" '+(isPO?'readonly ':'')+'style="'+(isPO?RO:IS)+'"></td>'
            +'<td style="padding:3px 4px;width:55px;vertical-align:top"><input type="number" class="grn-li-inp grn-pkgs-inp" data-gi="'+i+'" value="'+(line.packages||'')+'" placeholder="—" min="1" step="1" style="'+IS+';text-align:center;font-family:var(--font-mono);font-weight:700" title="No. of Packages" oninput="grnCalcLineTotal('+i+')"></td>'
            +'<td style="padding:5px 4px;text-align:center;vertical-align:top;padding-top:8px;color:var(--muted);font-size:13px;font-weight:300">&times;</td>'
            +'<td style="padding:3px 4px;width:80px;vertical-align:top"><input type="number" class="grn-li-inp grn-qpp-inp" data-gi="'+i+'" value="'+(line.qty_per_pkg||'')+'" placeholder="0.000" min="0" step="0.001" style="'+IS+';text-align:right" oninput="grnCalcLineTotal('+i+')"></td>'
            +'<td style="padding:5px 4px;width:48px;text-align:center;vertical-align:top;padding-top:8px"><span class="grn-uom-cell" data-gi="'+i+'" style="font-size:10.5px;font-weight:600;color:var(--muted)">'+escHtml(line.uom||'KG')+'</span></td>'
            +'<td style="padding:5px 4px;width:80px;text-align:right;vertical-align:top;padding-top:8px"><span class="grn-total-cell" data-gi="'+i+'" style="font-size:11.5px;font-weight:800;color:var(--teal);font-family:var(--font-mono)">'+((line.total_qty||line.received_qty)>0?(parseFloat(line.total_qty||line.received_qty)||0).toLocaleString('en-IN',{maximumFractionDigits:3}):'\u2014')+'</span></td>'
            +'<td style="padding:3px 4px;width:75px;vertical-align:top"><input type="number" class="grn-li-inp grn-poqty-inp" data-gi="'+i+'" value="'+(displayQty !== null ? displayQty : '')+'" readonly style="'+poQtyStyle+'" tabindex="-1"'+(poQtyTitle?' title="'+escHtml(poQtyTitle)+'"':'')+' ></td>'
...
document.addEventListener('input', function(e){
    ...
    if (isNaN(idx)) return;
    if (inp.classList.contains('grn-pkgs-inp')) { _grnLines[idx].packages = inp.value; grnCalcLineTotal(idx); return; }
    if (inp.classList.contains('grn-qpp-inp'))  { _grnLines[idx].qty_per_pkg = inp.value; grnCalcLineTotal(idx); return; }
    if (inp.classList.contains('grn-mat-inp')) {
        _grnLines[idx].material = inp.value;
        // Auto-fill UOM from material master
        var matRow = (_allRows||[]).find(function(r){ return (r.material_name||'').toLowerCase() === inp.value.toLowerCase(); });
        if (matRow && matRow.uom) {
            _grnLines[idx].uom = matRow.uom;
            var uomCell = document.querySelector('#grnLinesTbody tr[data-gi="'+idx+'"] .grn-uom-cell');
            if (uomCell) uomCell.textContent = matRow.uom;
        }
    }
...
async function saveGrn() {
    ...
    var payload = {
        ...
        voucher_type_name: (document.getElementById('grnVoucherType') ? document.getElementById('grnVoucherType').value : '') || null,
    ...
    <div style="background:var(--surface);min-height:100%">
        ...
        <div class="form-card" style="margin:14px 16px 0;border-radius:10px">
            ...
            <div class="form-card-body" style="padding:12px 14px">
                <div style="display:grid;grid-template-columns:200px 160px 1fr;gap:12px;margin-bottom:10px">
                    <div class="form-group">
                        <label class="form-label">GRN Type <span class="req">*</span></label>
                        <select class="form-input-styled combo" id="grnVoucherType" onchange="grnVoucherTypeChange()">
                    ...
                    <div class="form-group">
                        ...
                        <datalist id="grnSupplierList">${(_supRows||[]).map(function(s){ return '<option value="'+escHtml(s.supplier_name)+'">'; }).join('')}</datalist><datalist id="grnMatList"></datalist>
                    </div>
                </div>
                <div style="display:grid;grid-template-columns:200px 1fr;gap:12px;margin-bottom:10px">
                    <div class="form-group">
                        ...
                        <select class="form-input-styled combo" id="grnPoSelect" onchange="grnPoChange()">
                ...
                <table style="width:100%;border-collapse:collapse;font-size:12.5px">
                    <thead>
                        <tr style="background:var(--surface2);border-bottom:1px solid var(--border2)">
                            ...
                            <th style="padding:7px 4px;text-align:center;font-size:9px;font-weight:700;color:var(--muted);text-transform:uppercase;width:55px">Pkgs</th>
                            <th style="padding:7px 4px;text-align:center;font-size:9px;font-weight:700;color:var(--muted);text-transform:uppercase;width:14px">&times;</th>
                            <th style="padding:7px 4px;text-align:right;font-size:9px;font-weight:700;color:var(--muted);text-transform:uppercase;width:80px">Qty/Pkg</th>
                            <th style="padding:7px 4px;text-align:center;font-size:9px;font-weight:700;color:var(--muted);text-transform:uppercase;width:48px">UOM</th>
                            <th style="padding:7px 4px;text-align:right;font-size:9px;font-weight:700;color:var(--teal);text-transform:uppercase;width:80px">=Total</th>
                    ...
                    <tbody id="grnLinesTbody">
                        <tr><td colspan="16" style="padding:24px;text-align:center;color:var(--muted);font-size:12px">No items — select a PO above or click "+ Add Item"</td></tr>
                    </tbody>
                    <tfoot>
                        <tr style="border-top:1px solid var(--border)">
                            <td colspan="16" style="padding:5px 10px">
                        ...
                        <tr style="border-top:1px solid var(--border);background:var(--surface2)">
                            <td colspan="14" style="padding:7px 14px;font-size:10.5px;font-weight:600;color:var(--muted);text-align:right">Taxable Amount</td>
