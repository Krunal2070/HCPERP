...
function _supPopulateTypeDropdowns() {
    ...
    if (modalSel) {
        ...
        if (typeof comboboxRefresh === 'function') comboboxRefresh(modalSel);
    }
    ...
    if (filterSel) {
        ...
        if (typeof comboboxRefresh === 'function') comboboxRefresh(filterSel);
    }
}
...
function openSupModal(idxOrNull) {
    ...
    /* Populate State dropdown from _IND_STATES (defined in po.js) */
    var stateEl = document.getElementById('supModalState');
    if (stateEl && typeof _IND_STATES !== 'undefined') {
        ...
        stateEl.value = r ? (r.state||'') : '';
        if (typeof comboboxSyncDisplay === 'function') comboboxSyncDisplay(stateEl);
    }
...
function saveSupplier(addAnother) {
    ...
    var payload = {
        ...
        state:          (document.getElementById('supModalState')||{}).value || '',
    };
...
function openSupLedger(idx) {
    ...
    setTimeout(function(){
        ...
        if (sel) {
            sel.value = sup.supplier_name;
            if (typeof comboboxSyncDisplay === 'function') comboboxSyncDisplay(sel);
            supLedgerFilter();
        }
    }, 100);
}
...
function supClearFilters() {
    supSetFilter('all');
    var t = document.getElementById('supTypeFilter');    if(t) { t.value = ''; if(typeof comboboxSyncDisplay==='function') comboboxSyncDisplay(t); }
    var p = document.getElementById('supPayTypeFilter'); if(p) { p.value = ''; if(typeof comboboxSyncDisplay==='function') comboboxSyncDisplay(p); }
    supApplyFilters();
    supUpdateFilterDot();
}
