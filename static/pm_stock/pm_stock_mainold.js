...
function showToast(msg, type='success', ms){
  ...
  // Each toast gets a unique sequence number; only callbacks whose
  // sequence still matches the current one will act. Stale timers from
  // prior toasts become no-ops.
  const mySeq = ++_toastSeq;
  const duration = Math.max(800, Math.min(20000, Number(ms) || 3500));

  t.innerHTML = `${icons[type]||''} ${msg}`;
  // Reset className first so the transition re-triggers when called
  // rapidly with the same `.show` class.
  t.className = 'toast';
  t.removeAttribute('style');   // wipe any stale inline styling
  // Force reflow so the browser sees the class change as two distinct
  // states, otherwise the transition wouldn't kick in.
  void t.offsetWidth;
  t.className = `toast ${type} show`;

  // Clear any prior timer; we're the freshest toast now.
  if(_toastTimer){ clearTimeout(_toastTimer); _toastTimer = null; }

  // Primary auto-hide.
  _toastTimer = setTimeout(() => {
    // Bail if a newer toast has already replaced us.
    if(mySeq !== _toastSeq) return;
    t.classList.remove('show');
    _toastTimer = null;
    // Safety net: if `.show` removal didn't visually clear the toast
    // (stalled transition, devtools-stamped inline styles), force it
    // a tick later by hard-resetting the className and styles.
    setTimeout(() => {
      if(mySeq !== _toastSeq) return;
