    ...
    if (validLines.length) {
        ...
        validLines.forEach(function(l, i){
            ...
            lines.push((i+1)+'. '+l.material.trim()
                +'  —  Qty: '+qty.toLocaleString('en-IN',{minimumFractionDigits:3})+''
...
async function poLoadVoucherTypes(currentTypeName) {
    ...
    // Auto-select first if new PO and nothing selected
    if (!currentTypeName && _poVoucherTypes.length) {
        sel.value = _poVoucherTypes[0].name;
    }
