...
function openManualFormCreator(){
    ...
    const brandOpts='<option value="">— No Brand —</option>'
        +_fvqBrands.map(b=>'<option value="'+b.id+'">'+escHtml(b.name)+'</option>').join('');
    const mb=document.getElementById('manualFormBrand');   if(mb) mb.innerHTML=brandOpts;
    const lb=document.getElementById('manualLinkBrand');   if(lb) lb.innerHTML=brandOpts;
...
/* Switch between "Create New" and "Link to Existing" panes */
function _manualSetMode(mode){
    ...
    if(btnL){
        btnL.style.background = !isCreate?'rgba(139,92,246,.85)':'transparent';
        btnL.style.color      = !isCreate?'#fff':'var(--muted)';
    }
    // Toggle the panes
    ...
    if(metaCreate) metaCreate.style.display = isCreate?'grid':'none';
    if(inpModeBar) inpModeBar.style.display = isCreate?'flex':'none';
    if(ingWrap)    ingWrap.style.display    = isCreate?'block':'none';
    if(linkPane)   linkPane.style.display   = isCreate?'none':'block';
    if(footLeft)   footLeft.style.display   = isCreate?'flex':'none';
    if(hint){
        hint.textContent = isCreate
            ? 'Build a new formulation from scratch by entering ingredients below.'
            : 'Pick an existing formulation — ingredients will be shared. Changes to the source propagate here.';
        hint.style.color = isCreate?'var(--muted)':'#a78bfa';
    }
    if(sub){
        sub.textContent = isCreate
            ? 'Manually add ingredients and set concentrations'
            : 'Link a new name to an existing formulation — shared ingredients';
    }
    if(saveBtn){
        saveBtn.textContent = isCreate ? '✓ Save Formulation' : '🔗 Create Link';
    }
}
