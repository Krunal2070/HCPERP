...
function costImpactEmail(){
    ...
    if(d.changes && d.changes.length > 1){
        ...
        d.changes.forEach(function(c, ci){
            ...
            var pct  = (diff!=null && c.old_rate>0) ? ' / +' + ((diff/c.old_rate)*100).toFixed(2) + '%' : '';
            var diffStr = diff!=null ? ' (' + (diff>=0?'+':'') + 'Rs.' + fmtNum(diff,4) + pct + ')' : '';
            body += (ci+1) + '. ' + c.material_name + '\n';
            body += '   Old Rate : ' + oldR + ' / KG\n';
            body += '   New Rate : ' + newR + ' / KG\n';
            body += '   Increase : Rs.' + fmtNum(Math.abs(diff||0),4) + ' / KG' + diffStr + '\n';
        });
    } else {
        const singleDiff = (d.old_rate!=null && d.new_rate!=null) ? d.new_rate - d.old_rate : null;
        const singlePct  = (singleDiff!=null && d.old_rate>0)
            ? ((singleDiff/d.old_rate)*100).toFixed(2) + '%' : null;
        body += 'Material : ' + mat + '\n';
        body += 'Old Rate : Rs.' + fmtNum(d.old_rate,4) + ' / KG\n';
        body += 'New Rate : Rs.' + fmtNum(d.new_rate,4) + ' / KG\n';
        if(singleDiff!=null){
            body += 'Increase : Rs.' + fmtNum(Math.abs(singleDiff),4) + ' / KG'
                + (singlePct ? ' (+' + singlePct + ')' : '') + '\n';
        }
    }
    body += 'Date     : ' + today + '\n';
    ...
    topBatch.forEach((b,i)=>{
        ...
        const pct = (d2!=null&&o>0) ? ' / ' + (d2>=0?'+':'') + ((d2/o)*100).toFixed(2) + '%' : '';
        const ds  = d2!=null ? ' (' + (d2>=0?'+':'') + 'Rs.' + fmtNum(d2,2) + pct + ')' : '';
...
function _ciDownloadExcel(d, batches, inclProc, mat, today){
    ...
    // Build header lines — handle single vs multi-material
    const isMulti = d.changes && d.changes.length > 1;
    const h1 = ['RATE INCREASE INTIMATION — ' + (isMulti ? 'Multiple Materials' : mat)];
    let h2txt = 'Date: ' + today + '  |  ';
    if(isMulti){
        h2txt += d.changes.map(function(c){
            var diff = (c.old_rate!=null&&c.new_rate!=null) ? c.new_rate - c.old_rate : null;
            var pct  = (diff!=null&&c.old_rate>0) ? ' (+' + ((diff/c.old_rate)*100).toFixed(2) + '%)' : '';
            return c.material_name
                + ': Rs.' + fmtNum(c.old_rate,4)
                + ' -> Rs.' + fmtNum(c.new_rate,4)
                + (diff!=null ? ' (+Rs.' + fmtNum(diff,4) + ')' : '')
                + pct;
        }).join('   |   ');
    } else {
        var singleDiff = (d.old_rate!=null&&d.new_rate!=null) ? d.new_rate - d.old_rate : null;
        var singlePct  = (singleDiff!=null&&d.old_rate>0)
            ? ' (+' + ((singleDiff/d.old_rate)*100).toFixed(2) + '%)' : '';
        h2txt += 'Old Rate: Rs.' + fmtNum(d.old_rate,4)
            + '  |  New Rate: Rs.' + fmtNum(d.new_rate,4)
            + (singleDiff!=null ? '  |  Increase: Rs.' + fmtNum(singleDiff,4) + singlePct : '');
    }
    const h2 = [h2txt];
    ...
    if(inclProc){
        ...
        if(pb.length){
            const procRows = [h1, h2, [],
                ['#','Batch Name','Batch Size (KG)','Old Cost (Rs.)','New Cost (Rs.)','Diff (Rs.)','Diff (%)']
            ];
            pb.forEach((b,i)=>{
                const diff   = b.cost_diff||0;
                const oldC   = b.old_total_cost||0;
                const procPct = oldC>0 ? ((diff/oldC)*100).toFixed(2) + '%' : '—';
                procRows.push([i+1, b.batch_name,
                    +String(b.batch_size).replace(/[^\d.]/g,''),
                    +fmtNum(oldC,2).replace(/,/g,''),
            ...
            ws2['!cols'] = [{wch:4},{wch:44},{wch:16},{wch:20},{wch:20},{wch:14},{wch:10}];
            ws2['!merges'] = [{s:{r:0,c:0},e:{r:0,c:6}},{s:{r:1,c:0},e:{r:1,c:6}}];
            XLSX.utils.book_append_sheet(wb, ws2, 'Procurement Cost');
        }
    }

    const matSlug = isMulti ? 'Multi_Material' : mat.replace(/[^a-zA-Z0-9]/g,'_');
    const fname = 'Rate_Impact_' + matSlug + '_' + new Date().toISOString().slice(0,10) + '.xlsx';
