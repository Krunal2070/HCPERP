...
function poSetFilter(f) {
    _poFilter = f;
    document.querySelectorAll('[id^="poF"]').forEach(function(el){ el.classList.remove('active'); });
    var btnId = f==='all' ? 'poFAll' : f==='not_approved' ? 'poFNot_approved' : 'poF'+f.charAt(0).toUpperCase()+f.slice(1);
    var btn = document.getElementById(btnId); if(btn) btn.classList.add('active');
    ...
    if (!rows.length) {
        ...
        document.getElementById('poTbody').innerHTML = rows.map(function(r, i) {
            ...
            return '<tr data-po-idx="' + absIdx + '" ondblclick="openPoModalByIdx(' + absIdx + ')" style="cursor:pointer;' + (isCancelled ? 'opacity:0.6;' : '') + '" title="' + (isClosed ? 'Closed — double-click to view & print' : isCancelled ? 'Cancelled' : 'Double-click to edit') + '">'
                ...
                + '<td style="font-size:12px">' + (r.item_count ? r.item_count + ' item(s)' : (escHtml(r.material||'—'))) + '</td>'
...
function poRenderLines(focusIdx) {
    ...
    if (!tb) return;
    if (!_poLines.length) {
        tb.innerHTML = '<tr><td colspan="7" style="padding:24px;text-align:center;color:var(--muted);font-size:12px">No items \u2014 click &ldquo;+ Add Item&rdquo; above</td></tr>';
    ...
    tb.innerHTML = _poLines.map(function(line, i) {
        ...
        return '<tr style="border-bottom:1px solid var(--border);background:' + (i % 2 ? 'var(--surface2)' : 'var(--surface)') + '" data-li="' + i + '">'
            ...
            + '</td>'
            + '<td style="padding:4px 8px;width:120px"><input type="number" class="po-li-inp po-qty-inp po-li-calc" data-li="' + i + '" data-field="qty" value="' + (line.qty || '') + '" placeholder="0.000" min="0" step="0.001" style="' + MS + ';width:100%"></td>'
        ...
        fetch('/api/procurement/po/get?id=' + row.id)
            ...
            .then(function(d){
                ...
                _poLines = (o.items||[]).map(function(i){
                    return {material:i.material||'', qty:i.qty||'', rate:i.rate||''};
                });
...
async function savePoModal() {
    ...
    var payload = {
        ...
        items: validLines.map(function(l){
            return { material: l.material.trim(), qty: parseFloat(l.qty)||0, rate: parseFloat(l.rate)||0 };
        })
    };
