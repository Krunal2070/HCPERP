...
function _fvqSyncBulkBar(){
    ...
    ['fvqBulkBrandSel','fvqBulkBrandBarSel'].forEach(sid=>{
        const s=document.getElementById(sid); if(s&&any) s.innerHTML=opts;
    });
}
...
.hdr{ margin-bottom:3mm; display:flex; justify-content:space-between; align-items:flex-start; }
...
.meta{ display:flex; gap:6mm; align-items:center; padding:2mm 4mm; background:#f8fafc;
    border:1pt solid #e2e8f0; border-radius:3px; margin-bottom:3mm; font-size:7pt; }
...
thead th{ padding:2mm 2.5mm; font-size:6.5pt; font-weight:700; letter-spacing:.8px;
...
tbody td{ padding:1.2mm 2.5mm; vertical-align:middle; border-right:.5pt solid #eef0f4; overflow:hidden; white-space:nowrap; }
...
tr.tot td{ padding:1.8mm 2.5mm; background:${isCosting?'#fffbeb':'#f0fdf4'};
...
.ftr{ margin-top:3mm; padding-top:2mm; border-top:1pt solid #e2e8f0;
...
<div class="hdr">
    <div class="hdr-left">
        ...
        <div class="ti">${batchName}</div>
        ${productCode?`<div class="pc">Product Code: <strong>${productCode}</strong></div>`:''}
    </div>
    <div class="hdr-right">
...
<div class="meta">
    ...
    ${productCode?`<div class="meta-sep"></div><div class="meta-item"><span class="ml">Product Code</span><span class="mv">${productCode}</span></div>`:''}
...
+'<style>.mp-body table{border-collapse:collapse;width:100%;font-size:7pt;table-layout:fixed;margin:1mm 0}.mp-body td,.mp-body th{border:.75pt solid #cbd5e1;padding:1mm 2mm;vertical-align:top;word-wrap:break-word}.mp-body th{background:#f1f5f9;font-weight:700;text-align:center}</style>'
+'<div class="mp-body" style="font-size:7pt;color:#1a2035;line-height:1.5">'+manufProcess+'</div>'
+'</div>'
) : ''}
<div class="ftr">
    <div>
...
function _renderBrandList(){
    ...
    if(!el) return;
    if(!_fvqBrands.length){
        el.innerHTML='<div style="padding:24px;text-align:center;color:var(--muted);font-size:12px">No brands yet — add one below</div>';
        return;
    }
    el.innerHTML=_fvqBrands.map(b=>{
        const nm=escHtml(b.name);
        const safe=b.name.replace(/'/g,"\\'");
        const c=b.color||'#6366f1';
        const tc=b.text_color||'#ffffff';
        return '<div style="display:flex;align-items:center;gap:12px;padding:10px 18px;border-bottom:1px solid var(--border);transition:background .12s" onmouseover="this.style.background=\'var(--text-05)\'" onmouseout="this.style.background=\'\'">'
            +'<div style="width:16px;height:16px;border-radius:50%;background:'+c+';flex-shrink:0;box-shadow:0 0 0 3px '+c+'33"></div>'
            +'<span style="flex:1;font-size:12.5px;font-weight:600;color:var(--text)">'+nm+'</span>'
            +'<span style="font-size:10px;font-weight:700;padding:1px 8px;border-radius:20px;background:'+c+';color:'+tc+';border:1px solid '+c+'55;margin-right:4px">'+nm+'</span>'
            +'<button onclick="editBrand('+b.id+',\''+safe+'\',\''+c+'\',\''+tc+'\')" class="btn-ghost" style="height:28px;padding:0 12px;font-size:11px">Edit</button>'
            +'<button onclick="deleteBrand('+b.id+',\''+safe+'\')" class="btn-ghost" style="height:28px;padding:0 12px;font-size:11px;color:var(--red-text);border-color:rgba(244,63,94,.35)">&#10005; Delete</button>'
            +'</div>';
    }).join('');
}
...
function _populateBrandSelects(){
    ...
    const opts2='<option value="">— No Brand —</option>'
        +_fvqBrands.map(b=>'<option value="'+b.id+'">'+escHtml(b.name)+'</option>').join('');
    document.querySelectorAll('.fvq-brand-assign-sel').forEach(s=>{const v=s.value;s.innerHTML=opts2;s.value=v;});

    const mb=document.getElementById('manualFormBrand');
    if(mb){mb.innerHTML=opts2;}
}
...
function toggleFvqMenu(id){
    ...
    if(!open && id==='fvqBrandsMenu'){
        const opts='<option value="">— No Brand —</option>'
            +(_fvqBrands||[]).map(b=>'<option value="'+b.id+'">'+escHtml(b.name)+'</option>').join('');
        ['fvqBulkBrandSel','fvqBulkBrandBarSel'].forEach(sid=>{
            const s=document.getElementById(sid); if(s) s.innerHTML=opts;
        });
    }
...
async function addBrand(){
    ...
    try{
        ...
        await loadBrands();
        _brandMgrPage=Math.ceil(_fvqBrands.length/_BRAND_PAGE_SZ)||1;
        fvqRenderTable();
    }catch(e){toast('Error: '+e.message,'error');}
}
...
async function editBrand(id, oldName, oldColor, oldTextColor){
    ...
    document.querySelectorAll('#brandMgrList > div').forEach(row=>{
        if(row.innerHTML.includes('editBrand('+id+',')){
            row.id=rowId;
            ...
            row.innerHTML=
                ...
                +'<input id="'+ni+'" type="text" value="'+escHtml(oldName)+'" maxlength="200" '
                    +'style="flex:1;height:32px;padding:0 9px;border-radius:5px;border:1px solid var(--teal-dim);background:var(--surface);color:var(--text);font-size:12px;outline:none">'
...
════════════════════════════════════════════════════════ */
// _manualIngredients declared at top

function openManualFormCreator(){
    _manualIngredients=[];
    ['manualFormName','manualFormCode','manualFormSize'].forEach(id=>{
        const el=document.getElementById(id); if(el) el.value='';
    });
    const mb=document.getElementById('manualFormBrand');
    if(mb) mb.innerHTML='<option value="">— No Brand —</option>'
        +_fvqBrands.map(b=>'<option value="'+b.id+'">'+escHtml(b.name)+'</option>').join('');
...
function _renderManualIngRows(){
    ...
    if(!_manualIngredients.length){
        tbody.innerHTML='<tr><td colspan="5" style="padding:20px;text-align:center;color:var(--muted);font-size:12px">No ingredients yet — click <strong>+ Add Row</strong></td></tr>';
        return;
    }
    tbody.innerHTML=_manualIngredients.map((ing,i)=>{
        return '<tr style="border-bottom:1px solid var(--border)">'
            ...
            +'<td style="padding:4px 6px"><input value="'+escHtml(ing.material_name)+'" oninput="_manualIngredients['+i+'].material_name=this.value" class="form-input" placeholder="Material name…" style="width:100%;height:30px;font-size:12px"></td>'
            ...
            +'<td style="padding:4px 6px;width:110px"><input value="'+escHtml(ing.concentration||'')+'" oninput="_manualIngredients['+i+'].concentration=this.value" class="form-input" placeholder="% w/w" style="width:100%;height:30px;font-size:12px;color:var(--teal);text-align:right;font-family:var(--font-mono)"></td>'
...
async function saveManualFormulation(){
    ...
    try{
        const res=await fetch('/api/procurement/formulations/create_manual',{
            method:'POST',headers:{'Content-Type':'application/json'},
            body:JSON.stringify({batch_name,product_code,batch_size,brand_id,ingredients:validIngs})
        });
        const data=await res.json();
        if(data.status!=='ok') throw new Error(data.message);
        toast('Formulation "'+batch_name+'" created with '+validIngs.length+' ingredients','success',4000);
...
═══════════════════════════════════════════════════════ */
const _SECTION_KWS=['manufacturing process','product specification','mfg process'];
function _mpIsHeader(text){ const t=text.trim().toLowerCase(); return _SECTION_KWS.some(kw=>t===kw||t.startsWith(kw)); }
function _mpIsTitle(text){ 
    // A title is a non-numeric, non-header free text that appears before any steps
    // Typically the batch name like "AFRINEXT BEAUTY CREAM"
    const t=text.trim();
    return t.length>2 && !/^\d/.test(t) && !_mpIsHeader(t);
}
...
function _mpFormatHtml(html){
    ...
    function processNode(node){
        if(node.nodeType===Node.TEXT_NODE){ const t=node.textContent.trim(); if(t) blocks.push({type:'free',text:t}); return; }
        if(node.nodeName==='TABLE'){ blocks.push({type:'table',el:node.cloneNode(true)}); return; }
        if(node.nodeName==='OL'){ let n=1; node.querySelectorAll('li').forEach(li=>blocks.push({type:'step',num:n++,text:li.textContent.trim()})); return; }
        if(node.nodeName==='UL'){ node.querySelectorAll('li').forEach(li=>blocks.push({type:'free',text:li.textContent.trim()})); return; }
        if(['P','DIV','H1','H2','H3','H4','H5','H6','LI'].includes(node.nodeName)){
            const t=node.textContent.trim(); if(!t){ blocks.push({type:'spacer'}); return; }
            if(_mpIsHeader(t)){ blocks.push({type:'col-header',text:t}); return; }
            const m=t.match(/^(\d+)[.)\s]\s*(.*)/s);
            if(m && m[2].trim()) blocks.push({type:'step',num:parseInt(m[1]),text:m[2].trim()});
            else blocks.push({type:'free',text:t});
            return;
        }
        if(node.nodeName==='BR'){ blocks.push({type:'spacer'}); return; }
        node.childNodes.forEach(processNode);
    }
    tmp.childNodes.forEach(processNode);

    // Remove consecutive/leading/trailing spacers
    const cleaned=blocks.filter((b,i,arr)=>{
        if(b.type==='spacer'&&(i===0||i===arr.length-1)) return false;
        if(b.type==='spacer'&&arr[i-1]?.type==='spacer') return false;
        return true;
    });
    const useful=cleaned.filter(b=>b.type!=='spacer');
    if(!useful.length) return html;

    // ── Identify title and column header from leading non-step blocks ──
    // Pattern: [title?] [col-header?] [steps...]
    // The title is the first free block before any steps (batch name)
    // The col-header is a block matching "Manufacturing Process" etc.
    let titleText='';
    let colHeaderText='Manufacturing Process';
    let stepStart=0;

    for(let i=0;i<cleaned.length;i++){
        const b=cleaned[i];
        if(b.type==='step') { stepStart=i; break; }
        if(b.type==='col-header'){ colHeaderText=b.text; stepStart=i+1; }
        else if(b.type==='free'&&!titleText&&_mpIsTitle(b.text)){ titleText=b.text; stepStart=i+1; }
        else if(b.type==='spacer'){ stepStart=i+1; }
    }

    // Collect remaining blocks after the header section
    const remaining=cleaned.slice(stepStart>0?stepStart-1:0);

    // Build output
    const parts=[];
    let stepBuffer=[];
    let pendingTitle=titleText;
    let pendingColHeader=colHeaderText;

    function flushSteps(){
        if(!stepBuffer.length) return;
        parts.push(_mpBuildStepTable(stepBuffer, pendingTitle, pendingColHeader));
        pendingTitle=''; // only show title on first table
        stepBuffer=[];
    }

    // Process remaining blocks
    cleaned.forEach((b,i)=>{
        if(b.type==='step'){ stepBuffer.push(b); return; }
        if(b.type==='col-header'||( b.type==='free'&&b===cleaned[0]&&titleText===b.text)) return; // consumed
        if(b.type==='spacer') return;
        flushSteps();
        if(b.type==='table'){
            parts.push(_mpRestyleTable(b.el));
        } else if(b.type==='free'&&b.text.trim()&&b.text!==titleText){
            // Sub-section label
            if(_mpIsTitle(b.text)&&!stepBuffer.length){
                pendingTitle=b.text; // treat as new section title
            } else {
                parts.push('<div style="padding:2px 0;font-size:12px">'+escHtml(b.text)+'</div>');
            }
        }
    });
    flushSteps();
...
function _mpBuildStepTable(steps, title, colHeader){
    colHeader=colHeader||'Manufacturing Process';
    let html='<table style="border-collapse:collapse;width:100%;font-size:12px;table-layout:fixed;margin-bottom:0">'
        +'<colgroup><col style="width:32px"><col></colgroup>';
    // Title row (batch name) — spans both columns
    if(title){
        html+='<tr><td colspan="2" style="border:1px solid #cbd5e1;padding:5px 8px;font-weight:700;font-size:12px;text-align:center;background:#f8fafc">'+escHtml(title)+'</td></tr>';
    }
    // Column header row — blank # col, label in step col
    html+='<tbody>';
    steps.forEach(s=>{
        html+='<tr>'
            +'<td style="border:1px solid #cbd5e1;padding:4px 6px;text-align:center;color:#64748b;font-size:11px;vertical-align:top;white-space:nowrap">'+s.num+'</td>'
            +'<td style="border:1px solid #cbd5e1;padding:4px 8px;font-size:12px;vertical-align:top;line-height:1.5">'+escHtml(s.text)+'</td>'
            +'</tr>';
    });
    html+='</tbody></table>';
    return html;
...
function _mpRestyleTable(tableEl){
    tableEl.style.cssText='border-collapse:collapse;width:100%;font-size:12px;table-layout:fixed;margin-bottom:2px';
    tableEl.querySelectorAll('th').forEach(th=>{ th.style.cssText='border:1px solid #cbd5e1;padding:4px 8px;background:#f1f5f9;font-weight:700;text-align:center;font-size:11px;vertical-align:middle'; });
    tableEl.querySelectorAll('td').forEach(td=>{ const isSerial=td.cellIndex===0&&/^\d+\.?$/.test(td.textContent.trim()); td.style.cssText=isSerial?'border:1px solid #cbd5e1;padding:4px 4px;text-align:center;color:#64748b;font-size:11px;vertical-align:top;width:32px':'border:1px solid #cbd5e1;padding:4px 8px;font-size:12px;vertical-align:top;line-height:1.5'; });
    const w=document.createElement('div'); w.appendChild(tableEl); return w.innerHTML;
}
...
async function mpFormatSelected(){
    ...
    if(checked.length){
        ...
        if(!confirm('Auto-format manufacturing process for '+targets.length+' selected batch'+(targets.length!==1?'es':'')+'?')) return;
    } else {
        ...
        if(!confirm('No rows selected — auto-format ALL '+targets.length+' batches with a manufacturing process?')) return;
    }
    toast('Formatting '+targets.length+' batch'+(targets.length!==1?'es':'')+'…','info',3000);
