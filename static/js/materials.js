...
function buildRows(rows){
    ...
    return rows.map(r=>`
        <tr data-mat='${escHtml(JSON.stringify(r))}' ondblclick="openEditFromRow(this)">
            ...
            <td>${r.sr_no}</td>
            <td class="td-name">${escHtml(r.material_name)}${r.uom && r.uom !== 'KG' ? `<span style="margin-left:6px;font-size:9px;font-weight:700;padding:1px 6px;border-radius:10px;background:rgba(245,158,11,.12);color:#d97706;vertical-align:middle">${escHtml(r.uom)}</span>` : ''}${r.description?`<br><span style="font-size:10px;font-style:italic;color:var(--muted);font-weight:400">${escHtml(r.description)}</span>`:''}</td>
            <td>${r.group_name?`<span style="padding:2px 8px;border-radius:10px;background:var(--text-05);font-size:10px;color:var(--teal);font-weight:600;white-space:nowrap">${escHtml(r.group_name)}</span>`:'<span class="td-dim">—</span>'}${r.mat_type_abbr?` <span style="padding:1px 6px;border-radius:8px;font-size:9px;font-weight:700;color:#fff;background:${r.mat_type_color||'#6b7280'};white-space:nowrap">${escHtml(r.mat_type_abbr)}</span>`:''}</td>
...
function setFilter(f){
    ...
    var allPill = document.getElementById('fAll');
    if(allPill) allPill.classList.toggle('active', f === 'all');
    // Update Filter button label
    var labels = {all:'', good:'Good', low:'Low', zero:'Zero', req_nonzero:'Req > 0', req_zero:'Req = 0'};
    var lbl = document.getElementById('matFilterLabel');
    if(lbl) lbl.textContent = labels[f] || '';
    var dropBtn = document.getElementById('matFilterDropBtn');
...
document.addEventListener('keydown',e=>{
    ...
    if(e.altKey && !inInput){
        ...
        // Alt+Enter — edit focused/selected item
        if(e.key==='Enter'){
            e.preventDefault();
            // If a row is keyboard-focused, edit it
            if(_focusedIdx>=0 && getPageRows()[_focusedIdx]){
                openEditModal(getPageRows()[_focusedIdx]); return;
            }
            // If exactly one item is selected, edit it
            if(_selectedMats.size===1){
                const name=[..._selectedMats][0];
                const row=_allRows.find(r=>r.material_name===name);
                if(row){ openEditModal(row); return; }
            }
            return;
        }
    }
...
function matUpdateDeleteBtn(){
    ...
    // Unified Assign dropdown button — enable when selection exists
    const assignBtn=document.getElementById('matAssignBtn');
    if(assignBtn){
        assignBtn.disabled=count===0;
        assignBtn.style.opacity=count>0?'1':'.4';
        assignBtn.style.cursor=count>0?'pointer':'not-allowed';
        // Keep inner buttons always enabled (they check selection themselves)
    }
    // Also keep individual assign buttons usable (they're inside the dropdown)
    ['matAssignGroupBtn','matAssignGstBtn','matAssignUomBtn','matAssignTypeBtn'].forEach(function(id){
        const b=document.getElementById(id);
        if(b){b.disabled=false;} // dropdown is only shown when count>0 via parent
    });
...
function openEditModal(r){
    ...
    // Material Type
    const _typeSel  = document.getElementById('editMatTypeId');
    const _typeHint = document.getElementById('editMatTypeHint');
    if(_typeSel){
        _populateMatTypeSelect(_typeSel, isNew ? null : (r.material_type_id ?? null));
        if(!isNew && r.material_type_id) _typeSel.value = String(r.material_type_id);
        if(_typeHint){
            if(!isNew && r.material_type_id){
                const t = _matTypes.find(x=>String(x.id)===String(r.material_type_id));
                _typeHint.textContent = t ? `✓ ${t.type_name}` : '';
                _typeHint.style.color = 'var(--green-text)';
            } else {
                _typeHint.textContent = isNew ? '⚠ Please select a material type' : '';
                _typeHint.style.color = 'var(--amber-text)';
            }
        }
    }
    // Material Group
    ...
    // Gate the Group dropdown on the selected Type (RM-only for now)
    _applyGroupEnabledState();
    ...
    // UOM field — new records show inline in name row, edit shows separate group
    const _uomNew  = document.getElementById('editUom');
    const _uomEdit = document.getElementById('editUomEdit');
    const _uomGrp  = document.getElementById('editUomGroup');
    const uomVal   = isNew ? 'KG' : (r.uom || 'KG');
    if(_uomNew)  _uomNew.value  = uomVal;
    if(_uomEdit) _uomEdit.value = uomVal;
    if(_uomGrp)  _uomGrp.style.display = isNew ? 'none' : 'block';
...
async function saveEditModal(){
    ...
    const payload={
        ...
        material_type_id:   (document.getElementById('editMatTypeId')||{}).value || null,
        ...
        uom:                ((document.getElementById('editUomEdit')||document.getElementById('editUom')||{}).value || 'KG'),
    };
    try{
...
/* ═══════════════════════ MATERIAL TYPES ═══════════════════════ */
let _matTypes = [];  // [{id, type_name, abbreviation, color}]

async function loadMatTypes(){
    try{
        const res  = await fetch('/api/procurement/material_types');
        const data = await res.json();
        if(data.status !== 'ok') return;
        _matTypes = data.types || [];
        _populateMatTypeSelect(document.getElementById('editMatTypeId'), null);
    }catch(e){}
}

function _populateMatTypeSelect(sel, selectedId){
    if(!sel) return;
    const prev = selectedId !== null ? selectedId : (sel.value || null);
    ...
    _matTypes.forEach(t => {
        const opt = document.createElement('option');
        opt.value       = t.id;
        opt.textContent = t.type_name + (t.abbreviation ? ` (${t.abbreviation})` : '');
        sel.appendChild(opt);
    });
    if(prev) sel.value = String(prev);
}

function matTypeSelectChange(){
    const sel  = document.getElementById('editMatTypeId');
    const hint = document.getElementById('editMatTypeHint');
    if(!sel || !hint) return;
    if(!sel.value){
        hint.textContent = '⚠ Please select a material type';
        hint.style.color = 'var(--amber-text)';
    } else {
        const t = _matTypes.find(x => String(x.id) === String(sel.value));
        hint.textContent = t ? `✓ ${t.type_name}` : '';
        hint.style.color = 'var(--green-text)';
    }
    // Group dropdown is only relevant for Raw Material — disable for others
    _applyGroupEnabledState();
}

/**
 * Enables the Material Group dropdown (and "+ New Group" button) only when the
 * currently selected Material Type has abbreviation 'RM' (Raw Material).
 * For all other types, the group select is disabled with an explanatory hint.
 */
function _applyGroupEnabledState(){
    const typeSel  = document.getElementById('editMatTypeId');
    const groupSel = document.getElementById('editGroupId');
    const groupHint= document.getElementById('editGroupHint');
    if(!typeSel || !groupSel) return;

    // Find the +New Group button (inline-onclick, no id) by walking from the select
    let newGroupBtn = null;
    const wrap = groupSel.closest('div[style*="display:flex"]') || groupSel.parentElement?.parentElement;
    if(wrap){
        newGroupBtn = wrap.querySelector('button[onclick*="openMatGroupModalFromEdit"]');
    }

    const t = typeSel.value
        ? _matTypes.find(x => String(x.id) === String(typeSel.value))
        : null;
    const isRM = !!(t && (t.abbreviation || '').toUpperCase() === 'RM');

    if(isRM){
        // Enabled state — restore normal behaviour
        groupSel.disabled = false;
        groupSel.style.opacity   = '';
        groupSel.style.cursor    = '';
        groupSel.style.background= '';
        if(newGroupBtn){
            newGroupBtn.disabled = false;
            newGroupBtn.style.opacity = '';
            newGroupBtn.style.cursor  = '';
        }
        // Refresh the hint based on current selection
        if(groupHint){
            if(!groupSel.value){
                groupHint.textContent = '⚠ Please select a material group';
                groupHint.style.color = 'var(--amber-text)';
            } else {
                const g = _matGroups.find(x => String(x.id) === String(groupSel.value));
                groupHint.textContent = g ? `✓ ${g.group_name}` : '';
                groupHint.style.color = 'var(--green-text)';
            }
        }
    } else {
        // Disabled state — group is not applicable
        groupSel.disabled = true;
        groupSel.style.opacity   = '0.55';
        groupSel.style.cursor    = 'not-allowed';
        groupSel.style.background= 'var(--surface2, #f3f4f6)';
        if(newGroupBtn){
            newGroupBtn.disabled = true;
            newGroupBtn.style.opacity = '0.5';
            newGroupBtn.style.cursor  = 'not-allowed';
        }
        if(groupHint){
            groupHint.textContent = typeSel.value
                ? '— Material Group applies to Raw Material only'
                : '';
            groupHint.style.color = 'var(--text3, #94a3b8)';
        }
    }
}

/* Quick-create Material Type modal */
function openNewMatTypeModal(){
    const modal = document.getElementById('matTypeModal');
    if(modal) modal.classList.add('open');
    const inp = document.getElementById('newMatTypeName');
    if(inp){ inp.value = ''; inp.focus(); }
    const abbr = document.getElementById('newMatTypeAbbr');
    if(abbr) abbr.value = '';
    const color = document.getElementById('newMatTypeColor');
    if(color) color.value = '#6b7280';
}

function closeNewMatTypeModal(){
    const modal = document.getElementById('matTypeModal');
    if(modal) modal.classList.remove('open');
}

async function saveNewMatType(){
    const name  = (document.getElementById('newMatTypeName')?.value || '').trim();
    const abbr  = (document.getElementById('newMatTypeAbbr')?.value || '').trim().toUpperCase();
    const color = document.getElementById('newMatTypeColor')?.value || '#6b7280';
    if(!name){ toast('Type name is required', 'error'); return; }
    try{
        const res  = await fetch('/api/procurement/material_types/save', {
            method:'POST', headers:{'Content-Type':'application/json'},
            body: JSON.stringify({ type_name: name, abbreviation: abbr, color: color })
        });
        const data = await res.json();
        if(data.status !== 'ok') throw new Error(data.message);
        toast('Material type created: ' + name, 'success');
        closeNewMatTypeModal();
        await loadMatTypes();
        // Auto-select the newly created type in the modal
        const sel = document.getElementById('editMatTypeId');
...
// Load groups and types when page loads
loadMatGroups();
loadMatTypes();
...
/* ═══════════════════════ EXPORT ═══════════════════════ */
function doExport(){
    const headers=['Material Name','UOM','Aliases','Description',
                   'Supplier Name','Supplier Code',
                   'Ordered Qty','MSL','Last Purchase Rate (₹)',
    ...
    ws['!cols']=[{wch:44},{wch:8},{wch:40},{wch:50},{wch:36},{wch:14},{wch:12},{wch:12},{wch:16},{wch:14},{wch:12},{wch:14},{wch:16},{wch:16}];
...
function downloadTemplate(){
    ...
    const instructions=[
        ...
        '← KG/G/L/ML/PCS/MT/TON/BOX/PKT (default: KG)',
        ...
        '← Opening balance qty',
        '← Min Stock Level',
        '← Rate in ₹ (used to auto-fill PO)',
        '← e.g. 2905','← 0/5/12/18/28','← Taxable/Exempt','← Goods/Services'];
    const wsData=[headers, instructions,
        ['Sodium Chloride','KG','NaCl, Salt, Rock Salt','Food-grade white crystalline powder','ABC Chemicals Pvt Ltd','SUP001','500','50','45.50','2501','5','Taxable','Goods'],
        ['Example Material B','ML','','','','','','','','','','','']];
    const wb=XLSX.utils.book_new();
    const ws=XLSX.utils.aoa_to_sheet(wsData);
    ws['!cols']=[{wch:44},{wch:8},{wch:40},{wch:50},{wch:36},{wch:14},{wch:12},{wch:12},{wch:16},{wch:14},{wch:12},{wch:14},{wch:16}];
    ...
    reader.onload=e=>{
        try{
            ...
            _importRows=raw.map(row=>({
                ...
                uom:            (row['UOM']||'').toString().trim().toUpperCase() || null,
                ...
                msl:            nv(row['MSL']),
                last_purchase_rate: nv(row['Last Purchase Rate (₹)']) || nv(row['Last Purchase Rate (₹/kg)']) || nv(row['Last Purchase Rate']),
...
/* ═══════════════════════════════════════════════
   BULK ASSIGN UOM
═══════════════════════════════════════════════ */
function openBulkAssignUom(){
    const sel = [..._selectedMats];
    if(!sel.length){ toast('Select at least one material','warning'); return; }
    const listEl = document.getElementById('bulkUomMatList');
    if(listEl) listEl.innerHTML =
        '<strong style="color:var(--text)">' + sel.length + ' material' + (sel.length>1?'s':'') + ' selected:</strong><br>' +
        sel.map(n=>{
            const row = _allRows.find(r=>r.material_name===n);
            const uom = row?.uom || 'KG';
            return '<span style="display:inline-flex;align-items:center;gap:4px;margin:2px 4px 0 0;padding:1px 8px;border-radius:10px;background:var(--text-08);font-size:10.5px">' + escHtml(n) + ' <span style="color:var(--teal);font-weight:600">' + escHtml(uom) + '</span></span>';
        }).join('');
    const selEl = document.getElementById('bulkUomSelect');
    if(selEl) selEl.value = '';
    document.getElementById('bulkUomModal').classList.add('open');
}
function closeBulkAssignUom(){ document.getElementById('bulkUomModal').classList.remove('open'); }
async function saveBulkAssignUom(){
    const uom = document.getElementById('bulkUomSelect').value;
    if(!uom){ toast('Please select a UOM','warning'); return; }
    const sel = [..._selectedMats];
    if(!sel.length){ toast('No materials selected','warning'); return; }
    try{
        const res = await fetch('/api/procurement/bulk_assign_uom',{
            method:'POST', headers:{'Content-Type':'application/json'},
            body: JSON.stringify({uom: uom, material_names: sel})
        });
        const data = await res.json();
        if(data.status!=='ok') throw new Error(data.message);
        toast('UOM ' + uom + ' assigned to ' + data.updated + ' material' + (data.updated!==1?'s':''), 'success');
        closeBulkAssignUom();
        sel.forEach(name=>{ const r=_allRows.find(x=>x.material_name===name); if(r) r.uom=uom; });
        _selectedMats.clear(); applyFilters(); loadData();
    }catch(e){ toast('Assign failed: '+e.message,'error'); }
}

/* ═══════════════════════════════════════════════
   BULK ASSIGN MATERIAL TYPE
═══════════════════════════════════════════════ */
function openBulkAssignMatType(){
    const sel = [..._selectedMats];
    if(!sel.length){ toast('Select at least one material','warning'); return; }

    // Populate material list preview
    const listEl = document.getElementById('bulkMatTypeMatList');
    if(listEl) listEl.innerHTML =
        '<strong style="color:var(--text)">' + sel.length + ' material' + (sel.length>1?'s':'') + ' selected:</strong><br>' +
        sel.map(n=>{
            const row = _allRows.find(r=>r.material_name===n);
            const typeName = row?.material_type || '';
            const typeAbbr = row?.mat_type_abbr || '';
            return '<span style="display:inline-flex;align-items:center;gap:4px;margin:2px 4px 0 0;padding:1px 8px;border-radius:10px;background:var(--text-08);font-size:10.5px">'
                + escHtml(n)
                + (typeName ? ' <span style="color:var(--teal);font-weight:600">' + escHtml(typeAbbr||typeName) + '</span>' : '')
                + '</span>';
        }).join('');

    // Populate type dropdown from _matTypes
    const selEl = document.getElementById('bulkMatTypeSelect');
    if(selEl){
        selEl.innerHTML = '<option value="">-- Select Type --</option>';
        _matTypes.forEach(t => {
            const opt = document.createElement('option');
            opt.value       = t.id;
            opt.textContent = t.type_name + (t.abbreviation ? ` (${t.abbreviation})` : '');
    ...
    document.getElementById('bulkMatTypeModal').classList.add('open');
}

function closeBulkAssignMatType(){
    document.getElementById('bulkMatTypeModal').classList.remove('open');
}

async function saveBulkAssignMatType(){
    const typeId = document.getElementById('bulkMatTypeSelect').value;
    if(!typeId){ toast('Please select a material type','warning'); return; }
    const sel = [..._selectedMats];
    if(!sel.length){ toast('No materials selected','warning'); return; }
    const typeInfo = _matTypes.find(t=>String(t.id)===String(typeId));
    try{
        const res = await fetch('/api/procurement/bulk_assign_material_type',{
            method:'POST', headers:{'Content-Type':'application/json'},
            body: JSON.stringify({ material_type_id: parseInt(typeId), material_names: sel })
        });
        const data = await res.json();
        if(data.status!=='ok') throw new Error(data.message);
        const typeName = typeInfo ? typeInfo.type_name : 'type';
        toast(typeName + ' assigned to ' + data.updated + ' material' + (data.updated!==1?'s':''), 'success');
        closeBulkAssignMatType();
        // Update in-memory rows immediately
        sel.forEach(name=>{
            const r = _allRows.find(x=>x.material_name===name);
            if(r && typeInfo){
                r.material_type_id  = typeInfo.id;
                r.material_type     = typeInfo.type_name;
                r.mat_type_abbr     = typeInfo.abbreviation || '';
                r.mat_type_color    = typeInfo.color || '#6b7280';
            }
        });
        _selectedMats.clear(); applyFilters(); loadData();
    }catch(e){ toast('Assign failed: '+e.message,'error'); }
...
/* ═══════════════════════════════════════════════
   ...
   Excel (Export/Import) and Assign (Group/GST/UOM)
═══════════════════════════════════════════════ */
function matToggleFilterDrop(e) {
    var drop = document.getElementById('matFilterDrop');
    if (!drop) return;
    var open = drop.style.display !== 'none';
    // Close other dropdowns
    ['matExcelDrop','matAssignDrop'].forEach(function(id) {
...
function matToggleExcelDrop(e) {
    var drop = document.getElementById('matExcelDrop');
    if (!drop) return;
    var open = drop.style.display !== 'none';
    // Close other dropdowns
    ['matFilterDrop','matAssignDrop'].forEach(function(id) {
        var d = document.getElementById(id); if (d) d.style.display = 'none';
    });
    drop.style.display = open ? 'none' : 'block';
    if (e) e.stopPropagation();
...
function matToggleAssignDrop(e) {
    var drop = document.getElementById('matAssignDrop');
    if (!drop) return;
    var open = drop.style.display !== 'none';
    // Close other dropdowns
    ['matFilterDrop','matExcelDrop'].forEach(function(id) {
        var d = document.getElementById(id); if (d) d.style.display = 'none';
    });
    drop.style.display = open ? 'none' : 'block';
    if (e) e.stopPropagation();
