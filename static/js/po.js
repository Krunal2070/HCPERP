...
function poRenderTable() {
    ...
    if (!rows.length) {
        document.getElementById('poTbody').innerHTML =
            '<tr><td colspan="13"><div class="state-box"><div class="state-icon">🛒</div><h3>No Purchase Orders</h3><p>Materials with suppliers will appear here.</p></div></td></tr>';
    } else {
        document.getElementById('poTbody').innerHTML = rows.map(function(r, i) {
            ...
            return '<tr data-po-idx="' + absIdx + '" ondblclick="openPoModalByIdx(' + absIdx + ')" style="cursor:pointer;' + (isCancelled ? 'opacity:0.6;' : '') + '" title="' + (isClosed ? 'Closed — double-click to view & print' : isCancelled ? 'Cancelled' : 'Double-click to edit') + '">'
                ...
                + '</td>'
                + '<td class="td-mono">' + (r.qty != null ? r.qty : '—') + '</td>'
                + '<td class="td-mono">' + (r.rate ? '₹ ' + r.rate : '—') + '</td>'
...
function poCalcLineTotal(idx) {
    var pkgsEl  = document.querySelector('#poLinesTbody tr[data-li="' + idx + '"] .po-pkgs-inp');
    var qtyEl   = document.querySelector('#poLinesTbody tr[data-li="' + idx + '"] .po-qty-inp');
    var totalEl = document.getElementById('poTotal_' + idx);
    if (!pkgsEl || !qtyEl) return;
    var pkgs   = parseFloat(pkgsEl.value)  || 0;
    var qtyPkg = parseFloat(qtyEl.value)   || 0;
    var total  = pkgs * qtyPkg;
    _poLines[idx].packages    = pkgsEl.value;
    _poLines[idx].qty_per_pkg = qtyEl.value;
    _poLines[idx].total_qty   = total;
    _poLines[idx].qty         = total > 0 ? String(total) : '';
    if (totalEl) {
...
function poRenderLines(focusIdx) {
    ...
    if (!tb) return;
    if (!_poLines.length) {
        tb.innerHTML = '<tr><td colspan="8" style="padding:24px;text-align:center;color:var(--muted);font-size:12px">No items \u2014 click &ldquo;+ Add Item&rdquo; above</td></tr>';
    ...
    tb.innerHTML = _poLines.map(function(line, i) {
        ...
        var amt = (parseFloat(line.qty) || 0) * (parseFloat(line.rate) || 0);
        ...
        return '<tr style="border-bottom:1px solid var(--border);background:' + (i % 2 ? 'var(--surface2)' : 'var(--surface)') + '" data-li="' + i + '">'
            ...
            + '</td>'
            + '<td style="padding:4px 8px;width:80px"><input type="number" class="po-li-inp po-pkgs-inp" data-li="' + i + '" data-field="packages" value="' + (line.packages||'') + '" placeholder="—" min="1" step="1" style="' + MS + ';width:100%;text-align:center;font-weight:700" title="No. of Packages" oninput="poCalcLineTotal(' + i + ')"></td>'
            + '<td style="padding:4px 8px;width:14px;text-align:center;color:var(--muted);font-size:13px;font-weight:300">&times;</td>'
            + '<td style="padding:4px 8px;width:110px"><input type="number" class="po-li-inp po-qty-inp" data-li="' + i + '" data-field="qty_per_pkg" value="' + (line.qty_per_pkg || '') + '" placeholder="0.000" min="0" step="0.001" style="' + MS + ';width:100%" oninput="poCalcLineTotal(' + i + ')"></td>'
            ...
            + '<td style="padding:4px 8px;width:110px;text-align:right"><span id="poTotal_' + i + '" style="font-size:12.5px;font-weight:800;color:var(--teal);font-family:var(--font-mono)">' + (line.total_qty > 0 ? line.total_qty.toLocaleString('en-IN',{maximumFractionDigits:3}) : '\u2014') + '</span></td>'
...
function poCalcGrandTotal() {
    var taxable = 0, totalCGST = 0, totalSGST = 0;
    _poLines.forEach(function(l){
        var amt = (parseFloat(l.qty)||0)*(parseFloat(l.rate)||0);
        taxable += amt;
        if (amt > 0) {
            // Use stored cgst_amount first, else calculate from material master
            if (l.cgst_amount) {
                totalCGST += parseFloat(l.cgst_amount);
                totalSGST += parseFloat(l.sgst_amount||l.cgst_amount);
            } else {
                var mat    = (_allRows||[]).find(function(r){ return (r.material_name||'').toLowerCase()===(l.material||'').trim().toLowerCase(); });
                var gstPct = (mat&&mat.gst_rate!=null) ? parseFloat(mat.gst_rate) : 0;
                if (gstPct > 0) {
                    var cgst = Math.round(amt*(gstPct/2)/100*100)/100;
                    totalCGST += cgst; totalSGST += cgst;
                }
            }
        }
    });
    // Add freight and packing charges to taxable base
    ...
    var grand = taxable + totalCGST + totalSGST;
    ...
    var payload = {
        ...
        items: validLines.map(function(l){
            return {
                ...
                qty:         parseFloat(l.total_qty||l.qty)||0,
...
function gdOpenEditGodown(idx) {
    ...
    html += '<div class="form-group"><label class="form-label">State <span class="req">*</span></label><select class="form-input" id="gdAddrState" onchange="gdAddrStateChanged()">'+stateOpts+'</select></div>';
    html += '<div class="form-group"><label class="form-label">City <span class="req">*</span></label><select class="form-input" id="gdAddrCity"><option value="">— Select State first —</option></select></div>';
    ...
    html += '</div>';
    html += '<div style="margin-bottom:12px"><select class="form-input" id="gdGdDefault" style="height:30px;padding:0 8px;font-size:11px;width:auto">';
    ...
    document.getElementById('gdGdDefault').value = g.is_default ? '1' : '0';
...
function gdLoadBillingForm() {
    ...
    if (billState) billState.innerHTML = stateOpts;
...
/* Validate billing/shipping before save */
...
function buildAddressWidget(prefix, inputClass) {
    ...
    <div class="form-group">
        ...
        <select class="${ic}" id="${prefix}AddrState" onchange="addrStateChange('${prefix}','${ic}')">
    ...
    <div class="form-group">
        ...
        <select class="${ic}" id="${prefix}AddrCity">
